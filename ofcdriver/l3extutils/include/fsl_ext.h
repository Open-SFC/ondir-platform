...
#define FSLXM_HEADER(CLASS, FIELD, LENGTH) \
    FSLXM_HEADER__(CLASS, FIELD, 0, LENGTH + FSLX_VENDORID_LEN)

#define FSLXM_HEADER_W(CLASS, FIELD, LENGTH) \
    FSLXM_HEADER__(CLASS, FIELD, 1, ((LENGTH) * 2 + FSLX_VENDORID_LEN))
...
enum fslxm_ofb_match_fields {
    FSLXM_OFB_SPI_ID        = 120, /** IPSec SA spi */
    FSLXM_OFB_SRCIP_RANGE   = 121,
    FSLXM_OFB_DSTIP_RANGE   = 122,
    FSLXM_OFB_UDP_SRC_PORT_RANGE = 123,
    FSLXM_OFB_UDP_DST_PORT_RANGE = 124,
    FSLXM_OFB_TCP_SRC_PORT_RANGE = 125,
    FSLXM_OFB_TCP_DST_PORT_RANGE = 126,
    FSLXM_OFB_TCPFLAGS	    = 127
};
enum fslx_tcp_flags{
    FSLX_TCP_BITS_FIN = 0x01,
    FSLX_TCP_BITS_SYN = 0x02,
    FSLX_TCP_BITS_RST = 0x04,
    FSLX_TCP_BITS_PSH = 0x08,
    FSLX_TCP_BITS_ACK = 0x10,
    FSLX_TCP_BITS_URG = 0x20
};
 
#define OF_OXM_HDR_LEN   (4)
/* Masking: Arbitrary masks. */
#define FSLXM_OF_SPI    	FSLXM_HEADER(0xFFFF, FSLXM_OFB_SPI_ID, 4)
#define FSLXM_OF_SRCIP_RANGE	FSLXM_HEADER(0xFFFF, FSLXM_OFB_SRCIP_RANGE, 8)
#define FSLXM_OF_DSTIP_RANGE	FSLXM_HEADER(0xFFFF, FSLXM_OFB_DSTIP_RANGE, 8)
#define FSLXM_OF_UDP_SRC_PORT_RANGE	FSLXM_HEADER(0xFFFF, FSLXM_OFB_UDP_SRC_PORT_RANGE, 4)
#define FSLXM_OF_UDP_DST_PORT_RANGE	FSLXM_HEADER(0xFFFF, FSLXM_OFB_UDP_DST_PORT_RANGE, 4)
#define FSLXM_OF_TCP_SRC_PORT_RANGE	FSLXM_HEADER(0xFFFF, FSLXM_OFB_TCP_SRC_PORT_RANGE, 4)
#define FSLXM_OF_TCP_DST_PORT_RANGE	FSLXM_HEADER(0xFFFF, FSLXM_OFB_TCP_DST_PORT_RANGE, 4)
#define FSLXM_OF_TCPFLAGS	FSLXM_HEADER_W(0xFFFF, FSLXM_OFB_TCPFLAGS, 1)
...
typedef struct fslx_instruction_actions {
    ...
    struct ofp_action_header actions[0];  /* Actions associated with
                                             OFPIT_WRITE_TABLE_ACTIONS and
                                             OFPIT_APPLY_TABLE_ACTIONS */
}fslx_instruction_actions_t;
OFP_ASSERT(sizeof(struct fslx_instruction_actions) == 16);
...
enum fslx_action_subtype {
   ...
   /** struct fslx_action_tunnel_id_cntxt */
    ...
    /** struct fslx_action_validate_iphdr */
    FSLXAT_VALIDATE_IP_HDR	    = 31,
    /** struct fslx_action_validate_xport_hdr */
    FSLXAT_VALIDATE_TRANS_HDR	    = 32,
    /** struct fslx_qos_obj*/
    FSLXAT_QOS_CONFIG_OBJ		= 33,
    /** struct fslx_action_bind_mod */
    FSLXAT_BIND_MOD			= 34
};
...
enum fslx_instrctn_subtype {
   ...
   FSLXIT_WRITE_PRIORITY_REG_FROM_NEXT_FLOW=11, //ksg
   /* struct fslxit_write_table_actions */
   FSLXIT_WRITE_TABLE_ACTIONS = 12,
   /* struct fslxit_apply_table_actions */
   FSLXIT_APPLY_TABLE_ACTIONS = 13
};
...
OFP_ASSERT(sizeof(struct fslxat_write_metadata) == 24);
...
/*  \struct fslxit_table_actions 
 *  \brief instruction is similar to write action instruction.\n
 *  <b>Description:</b>\n
 *  Actions of write action set will be executed at last table where as actions of table action set will be 
 *  executed at a given table. With this type of instruction a packet can carry actions to be executed by
 *  next tables in the pipe line.
 */
struct fslxit_table_actions {
       /** OFPIT_EXPERIMENTER */
       uint16_t type;
       ...
       /** FSLX_VENDOR_ID. */
       uint32_t vendor;
       /** FSLXIT_WRITE_TABLE_ACTIONS/FSLXIT_APPLY_TABLE_ACTIONS */
       uint16_t subtype;
       /** table identifier. */
       uint8_t table_id;
       /** Align to 64-bits */
...
OFP_ASSERT(sizeof(struct fslxit_table_actions) ==16);
...
#define FSLXIT_WRITE_TABLE_ACTIONS_LEN 		sizeof(struct fslxit_table_actions)
#define FSLXIT_APPLY_TABLE_ACTIONS_LEN 		sizeof(struct fslxit_table_actions)
...
#define FSLX_BIND_OBJ_STATIC  0 
#define FSLX_BIND_OBJ_DYNAMIC 1
   uint8_t    obj_type;
   uint8_t  pad[7];
   
   struct ofp_instruction instructions[0]; 
};
OFP_ASSERT(sizeof(struct fslx_bind_mod) == 56);

/** \in struct fslxat_bind_mod
 *  <b>Description:<b>\n
 *  This action is used to add/modify/delete bind objects in the OF Switch to bind autonomously learnt flow entries.
 */
struct fslxat_bind_mod     {
        uint16_t type; /* OFPAT_EXPERIMENTER */
        uint16_t len;  /* Length is padded to 64 bits. */
        uint32_t vendor; /* FSLX_VENDOR_ID. */
        uint16_t subtype; /* FSLXAT_BIND_MOD */
        uint8_t  pad[2];
        uint32_t hit_count;
        struct fslx_bind_mod bind_mod;
};
OFP_ASSERT(sizeof(struct fslxat_bind_mod) == 72);
...
enum fslx_bo_application_types {
  ...
  /**IPSEC application*/
  FSLX_BO_APP_IPSEC,
  /**connection template application*/
  FSLX_BO_APP_CONNTEMPLATE,
...
OFP_ASSERT(sizeof(struct fslx_action_attach_bind_obj) == 24);
...
/*  \struct fslxat_validate_iphdr
 *  \brief This is a table level action would be executed before table lookup for a perticular flow similar to ip reassembly action \n
 *  <b>Description:</b>\n
 *  This action validates complete IP header to prevent few attacks with invalid lengths.
 *  this action is per packet action would be executed for every packet comes to that perticular table where it has been installed. 
 */
struct fslxat_validate_iphdr	{
    /** OFPAT_EXPERIMENTER. */
    uint16_t type;                  
    ...
    /** FSLX_VENDOR_ID. */
    uint32_t vendor;                
    /** FSLXAT_VALIDATE_IP_HDR */
    uint16_t subtype;               
    /** Pad to 64 bits. */
    uint8_t  pad[6];                
}; 
OFP_ASSERT(sizeof(struct fslxat_validate_iphdr) == 16);


/*  \struct fslxat_validate_xport_hdr
 *  \brief This is a table level action would be executed before table lookup for a perticular flow similar to ip reassembly action \n
 *  <b>Description:</b>\n
 *  This action validates complete transport header to prevent few attacks with invalid lengths.
 *  If protocol value is given as 0xFF then this generic action validates any transport header which comes as IP payload.
 *  If protocol specific protocol value such as 1,2,6,17 etc., then this action check for that perticular transport header,
 *  if found validates else just return proceed without doing anything.
 *  This action is per packet action would be executed for every packet comes to that perticular table where it has been installed. 
 */
struct fslxat_validate_xport_hdr	{
    /** OFPAT_EXPERIMENTER. */
...
OFP_ASSERT(sizeof(struct fslxat_validate_xport_hdr) == 16);
...
#define FSLX_ACTION_WRITE_METADATA_LEN  sizeof(struct fslxat_write_metadata)
...
/** \Experimenter Action 
 *  FSLX_ACTION_VALIDATE_IP_HDR_LEN
 *   - Returns length of buffer required for 'validate ip hdr action'
 */
#define FSLX_ACTION_VALIDATE_IP_HDR_LEN		sizeof(struct fslxat_validate_iphdr)

/** \Experimenter Action 
 *  FSLX_ACTION_VALIDATE_TRANS_HDR_LEN
 *   - Returns length of buffer required for 'validate transport hdr action'
 */
#define FSLX_ACTION_VALIDATE_TRANS_HDR_LEN	sizeof(struct fslxat_validate_xport_hdr)
...
#define FSLX_ACTION_QOS_OBJ_LEN       sizeof(struct fslx_qos_obj)

#define FSLX_ACTION_BIND_OBJ_LEN          sizeof(struct fslxat_bind_mod)
...
#define FSLX_QOS_CEETM_NUM_MAX_Q 8
struct fslx_qos_obj{
        /** OFPAT_EXPERIMENTER. */
        uint16_t type;
        /** Length of this action */
        ...
        uint8_t    weight[FSLX_QOS_CEETM_NUM_MAX_Q];
        uint8_t    cr_map[FSLX_QOS_CEETM_NUM_MAX_Q];/*!< Each byte 0-7 indicates the Comitted Rate */
        uint8_t   er_map[FSLX_QOS_CEETM_NUM_MAX_Q]; /*!< Excess Rate Eligibilty of respective queue */
};
OFP_ASSERT(sizeof(struct fslx_qos_obj) == 72);

/* Freescale queue property description. */
struct fslx_queue_properties{
    //struct ofp_queue_prop_header prop_header; /* prop: OFPQT_EXPERIMENTER, len: 16. */
    uint16_t property;    /* OFPQT_EXPERIMENTER. */
    uint16_t len;         /* Length of property, including this header. */
    uint8_t pad[4];       /* 64-bit alignemnt. */
    uint32_t vendor;        /** FSLX_VENDOR_ID. */
    uint32_t ceetm_classq_id;       /* 64-bit alignment */
};
OFP_ASSERT(sizeof(struct fslx_queue_properties) == 16);


/**************** Freescale Qos Open Flow Extensions - End *****************************/
...
int32_t fslx_action_write_metadata(struct of_msg *msg, uint64_t data);
int32_t fslx_action_ipfragment(struct of_msg *msg, uint32_t mtu_reg,uint8_t offset, uint8_t n_bits, uint32_t pmtu_val);
int32_t fslx_action_send_icmpv4_time_exceed_err_msg(struct of_msg *msg, uint8_t error_code);
int32_t fslx_action_drop_packet(struct of_msg *msg);

/** fsxl extension match fields **/
int32_t fslx_set_udp_src_port_range(struct of_msg *msg, uint16_t *start_port, uint16_t *end_port);
int32_t fslx_set_udp_dst_port_range(struct of_msg *msg, uint16_t *start_port, uint16_t *end_port);
int32_t fslx_set_tcp_src_port_range(struct of_msg *msg, uint16_t *start_port, uint16_t *end_port);
int32_t fslx_set_tcp_dst_port_range(struct of_msg *msg, uint16_t *start_port, uint16_t *end_port);
int32_t fslx_set_src_ip_range(struct of_msg *msg, uint32_t *start_ip, uint32_t *end_ip);
int32_t fslx_set_dst_ip_range(struct of_msg *msg, uint32_t *start_ip, uint32_t *end_ip);
