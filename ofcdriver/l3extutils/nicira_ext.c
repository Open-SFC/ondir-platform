		    ...
		    )
{
	...
	if (( NXM_FIELD(reg_index) < 0 )|| (NXM_FIELD(reg_index) >  9))
	{
	...
	while(match_fields_len >0)
	{
		...
		OF_LOG_MSG(OF_LOG_EXT_NICIRA, OF_LOG_DEBUG, "parsed data len %d", parsed_data_len);
                if(match_fields_len >= parsed_data_len)
                {
		match_fields_len -= parsed_data_len;
                }
                else
                {
                  OF_LOG_MSG(OF_LOG_EXT_NICIRA, OF_LOG_ERROR, " Parsed datalength is more than matchfield length match_fields_len=%d parsed_data_len=%d ", match_fields_len,parsed_data_len); 
                  break;
                }
		ptr             += parsed_data_len;
	}
   ...
   if(is_mask_set)
   {
      ...
      switch(reg_index)
      {
         ...
         *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) = htonl(NXM_NX_REG7_W);
         break;          
	case MFF_REG8:
         *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) = htonl(NXM_NX_REG8_W);
         break;
	case MFF_REG9:
         *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) = htonl(NXM_NX_REG9_W);
         break; 
      }
