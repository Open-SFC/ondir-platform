...
{
	struct of_msg_descriptor *msg_desc;
	struct fslxat_write_metadata *wr_metadata;
	...
	wr_metadata =
		(struct fslxat_write_metadata *)(msg_desc->ptr3+msg_desc->length3);
...
int32_t
fslx_action_validate_ip_hdr(struct of_msg *msg)
{
	struct of_msg_descriptor *msg_desc;
	struct fslxat_validate_iphdr *val_iphdr;

	msg_desc = &msg->desc;
	if((msg_desc->ptr3+msg_desc->length3 + (FSLX_ACTION_VALIDATE_IP_HDR_LEN)) >
			(msg->desc.start_of_data + msg_desc->buffer_len)) {
		OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN, "Length of buffer is not suffeceient to add data");
		return OFU_NO_ROOM_IN_BUFFER;
	}
	...
	val_iphdr = 
		(struct fslxat_validate_iphdr *)(msg_desc->ptr3+msg_desc->length3);

	val_iphdr->type = htons(OFPAT_EXPERIMENTER);
	val_iphdr->len = htons(FSLX_ACTION_VALIDATE_IP_HDR_LEN);
	val_iphdr->vendor = htonl(FSLX_VENDOR_ID);
	val_iphdr->subtype = htons(FSLXAT_VALIDATE_IP_HDR);

	msg_desc->length3 += (FSLX_ACTION_VALIDATE_IP_HDR_LEN);
	return OFU_ACTION_PUSH_SUCCESS;
}

/* if protocol value is oxff, it validates transport hdr received in the packet, 
   if protocol value is specific such as 1,2,6,17 etc. then this action check for that that perticular
   transport header, if found validatesit else just without doing anything return proceed 
*/
int32_t
fslx_action_validate_transport_hdr(struct of_msg *msg, uint8_t protocol)
{
	struct of_msg_descriptor *msg_desc;
	struct fslxat_validate_xport_hdr *val_transhdr;

	msg_desc = &msg->desc;
	if((msg_desc->ptr3+msg_desc->length3 + (FSLX_ACTION_VALIDATE_TRANS_HDR_LEN)) >
			(msg->desc.start_of_data + msg_desc->buffer_len)) {
		OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN, "Length of buffer is not suffeceient to add data");
		return OFU_NO_ROOM_IN_BUFFER;
	}
	...
	val_transhdr = 
		(struct fslxat_validate_xport_hdr *)(msg_desc->ptr3+msg_desc->length3);

	val_transhdr->type = htons(OFPAT_EXPERIMENTER);
	val_transhdr->len = htons(FSLX_ACTION_VALIDATE_TRANS_HDR_LEN);
	val_transhdr->vendor = htonl(FSLX_VENDOR_ID);
	val_transhdr->subtype = htons(FSLXAT_VALIDATE_TRANS_HDR);

	msg_desc->length3 += (FSLX_ACTION_VALIDATE_TRANS_HDR_LEN);
	return OFU_ACTION_PUSH_SUCCESS;
}

int32_t 
fslx_start_pushing_write_table_action_instruction(struct of_msg *msg, uint8_t table_id)
{
  struct of_msg_descriptor *msg_desc=NULL;
  struct fslxit_table_actions *operation=NULL;

  cntlr_assert(msg != NULL);
  msg_desc = &msg->desc;
  if ((msg_desc->ptr2+msg_desc->length2 + FSLXIT_WRITE_TABLE_ACTIONS_LEN) >
			(msg->desc.start_of_data + msg_desc->buffer_len)) 
  {
...
int32_t
fslx_end_pushing_table_action_instruction(struct of_msg *msg, uint16_t length_of_actions)
{
		...
		)
{
	...
	do
	{
		...
		msg->desc.data_len += (FSLX_TABLE_CONFIG_MSG_LEN + FSLX_HEADER_LEN);
...
/***************************************************************************
 * Function Name : fslx_set_src_ip_range
 * Description   : 
 * Parameter     : None 
 * Return value  : OFU_SET_FIELD_SUCCESS/OFU_NO_ROOM_IN_BUFFER
***************************************************************************/
int32_t fslx_set_src_ip_range(struct of_msg *msg, 
                              uint32_t *start_ip, 
                              uint32_t *end_ip)
{
  struct of_msg_descriptor *msg_desc = &msg->desc;

  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, "ENTRY");

  cntlr_assert(msg != NULL);
  msg_desc = &msg->desc;

  if ((msg_desc->ptr1+msg_desc->length1 + OFU_SRC_IP_RANGE_LEN) >
          (msg->desc.start_of_data + msg_desc->buffer_len)) 
  {
      OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
               "Length of buffer is not sufficient to add data");
  ...
  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) =\
                                   htonl(FSLXM_OF_SRCIP_RANGE);

  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN) =\
                                   htons(FSLX_VENDOR_ID);

  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN+4) =\
                                   htonl(*start_ip);

  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4+4) =\
                                   htonl(*end_ip); 
  msg_desc->length1 += OFU_SRC_IP_RANGE_LEN;
  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, "EXIT");
  return OFU_SET_FIELD_SUCCESS;
}

/***************************************************************************
 * Function Name : fslx_set_dst_ip_range
 * Description   : 
 * Parameter     : None 
 * Return value  : OFU_SET_FIELD_SUCCESS/OFU_NO_ROOM_IN_BUFFER
***************************************************************************/
int32_t fslx_set_dst_ip_range(struct of_msg *msg, 
                              uint32_t *start_ip, 
                              uint32_t *end_ip)
{
  struct of_msg_descriptor *msg_desc = &msg->desc;
  ...
  if ((msg_desc->ptr1+msg_desc->length1 + OFU_DEST_IP_RANGE_LEN) >
          (msg->desc.start_of_data + msg_desc->buffer_len)) 
  {
      OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
               "Length of buffer is not sufficient to add data");
  ...
  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4 + 4) =\
                                   htonl(*end_ip); 
  msg_desc->length1 += OFU_DEST_IP_RANGE_LEN;
  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, "EXIT");
  return OFU_SET_FIELD_SUCCESS;
}

/***************************************************************************
 * Function Name : fslx_set_src_port_range
 * Description   : 
 * Parameter     : None 
 * Return value  : OFU_SET_FIELD_SUCCESS/OFU_NO_ROOM_IN_BUFFER
***************************************************************************/
int32_t fslx_set_udp_src_port_range(struct of_msg *msg, 
                                    uint16_t *start_port, 
                                    uint16_t *end_port)
{
  struct of_msg_descriptor *msg_desc = &msg->desc;
  ...
  if ((msg_desc->ptr1+msg_desc->length1 + OFU_UDP_SRC_PORT_RANGE_LEN) >
          (msg->desc.start_of_data + msg_desc->buffer_len)) 
  {
      OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
               "Length of buffer is not sufficient to add data");
  ...
  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) = \
                                   htonl(FSLXM_OF_UDP_SRC_PORT_RANGE); 

  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN) =\
                                   htons(FSLX_VENDOR_ID); 

  *(uint16_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN +4) =\
                                   htons(*start_port); 

  *(uint16_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4+2) = \
                                   htons(*end_port); 

  msg_desc->length1 += OFU_UDP_SRC_PORT_RANGE_LEN;
  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, "EXIT");
  return OFU_SET_FIELD_SUCCESS;
}
...
/***************************************************************************
 * Function Name : fslx_set_dest_port_range
 * Description   : 
 * Parameter     : None 
 * Return value  : OFU_SET_FIELD_SUCCESS/OFU_NO_ROOM_IN_BUFFER
***************************************************************************/
int32_t fslx_set_udp_dst_port_range(struct of_msg *msg, 
                                    uint16_t *start_port, 
                                    uint16_t *end_port)
{
  struct of_msg_descriptor *msg_desc = &msg->desc;
  ...
  if ((msg_desc->ptr1+msg_desc->length1 + OFU_UDP_DEST_PORT_RANGE_LEN) >
          (msg->desc.start_of_data + msg_desc->buffer_len)) 
  {
      OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
               "Length of buffer is not sufficient to add data");
  ...
  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) =\
                                   htonl(FSLXM_OF_UDP_DST_PORT_RANGE); 
  ...
  *(uint16_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4 + 2) =\
  ...
  msg_desc->length1 += OFU_UDP_DEST_PORT_RANGE_LEN;

  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, "EXIT");
  return OFU_SET_FIELD_SUCCESS;
}
/***************************************************************************
 * Function Name : fslx_set_tcp_src_port_range
 * Description   : 
 * Parameter     : None 
 * Return value  : OFU_SET_FIELD_SUCCESS/OFU_NO_ROOM_IN_BUFFER
***************************************************************************/
int32_t fslx_set_tcp_src_port_range(struct of_msg *msg, 
                                    uint16_t *start_port, 
                                    uint16_t *end_port)
{
  struct of_msg_descriptor *msg_desc = &msg->desc;
  ...
  if ((msg_desc->ptr1+msg_desc->length1 + OFU_TCP_SRC_PORT_RANGE_LEN) >
          (msg->desc.start_of_data + msg_desc->buffer_len)) 
  {
      OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
               "Length of buffer is not sufficient to add data");
  ...
  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) =\
                                   htonl(FSLXM_OF_TCP_SRC_PORT_RANGE); 
  ...
  *(uint16_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4 + 2) = \
  ...
  msg_desc->length1 += OFU_TCP_SRC_PORT_RANGE_LEN;
  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, "EXIT");
  return OFU_SET_FIELD_SUCCESS;
}
...
/***************************************************************************
 * Function Name : fslx_set_tcp_dest_port_range
 * Description   : 
 * Parameter     : None 
 * Return value  : OFU_SET_FIELD_SUCCESS/OFU_NO_ROOM_IN_BUFFER
***************************************************************************/
int32_t fslx_set_tcp_dst_port_range(struct of_msg *msg, 
                                    uint16_t *start_port, 
                                    uint16_t *end_port)
{
  struct of_msg_descriptor *msg_desc = &msg->desc;
  ...
  if ((msg_desc->ptr1+msg_desc->length1 + OFU_TCP_DEST_PORT_RANGE_LEN) >
          (msg->desc.start_of_data + msg_desc->buffer_len)) 
  {
      OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
               "Length of buffer is not sufficient to add data");
  ...
  *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) =\
                                   htonl(FSLXM_OF_TCP_DST_PORT_RANGE); 
  ...
  *(uint16_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4 + 2) =\
  ...
  msg_desc->length1 += OFU_TCP_DEST_PORT_RANGE_LEN;
  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, "EXIT");
  return OFU_SET_FIELD_SUCCESS;
}
/***************************************************************************
 * Function Name : fslx_set_tcp_flags
 * Description   : 
 * Parameter     : None 
 * Return value  : OFU_SET_FIELD_SUCCESS/OFU_NO_ROOM_IN_BUFFER
***************************************************************************/
int32_t fslx_set_tcp_flags(struct of_msg *msg, 
                           uint8_t *tcp_flags, 
                           uint8_t is_mask_set, 
                           uint8_t *mask_val)
{
  ...
  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, 
             "tcp_flags:%x,is_mask_set:%d,mask_val:%x",
             tcp_flags,is_mask_set,mask_val);

  if(is_mask_set)
  {
    if ((msg_desc->ptr1+msg_desc->length1 + OFU_TCP_FLAGS_MASK_LEN) >
          (msg->desc.start_of_data + msg_desc->buffer_len)) 
    {
      OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
               "Length of buffer is not sufficient to add data");
    ...
    *(uint32_t*)(msg_desc->ptr1+msg_desc->length1) = htonl(FSLXM_OF_TCPFLAGS);

    *(uint32_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN)=
                                                 htonl(FSLX_VENDOR_ID);

    *(uint8_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4)=
                                                  *tcp_flags; 

    *(uint8_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4 + 1)=
                                                   *mask_val; 
    msg_desc->length1 += OFU_TCP_FLAGS_MASK_LEN;
  }
  else
  {
    if ((msg_desc->ptr1+msg_desc->length1 + OFU_TCP_FLAGS_LEN) >
          (msg->desc.start_of_data + msg_desc->buffer_len)) 
    {
      OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
               "Length of buffer is not sufficient to add data");
    ...
    *(uint8_t*)(msg_desc->ptr1+msg_desc->length1 + OF_OXM_HDR_LEN + 4)=
                                                  *tcp_flags; 
    msg_desc->length1 += OFU_TCP_FLAGS_LEN;
  }
  OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_INFO, "EXIT");
  return OFU_SET_FIELD_SUCCESS;
}
...
#if 0
int32_t fslx_instruction_write_table_actions(struct of_msg *msg,
			uint32_t table_id)
{
	 struct of_msg_descriptor *msg_desc;
	 struct fslxit_table_actions *table;
	 ...
	 msg_desc->ptr2= msg_desc->ptr2+msg_desc->length2;
	 msg_desc->length2=0;
	 table = (struct fslxit_table_actions *)(msg_desc->ptr2);
	 table->type = htons(OFPIT_EXPERIMENTER);
	 table->len  = htons(FSLXIT_WRITE_TABLE_ACTIONS_LEN);
	 table->vendor = htonl(FSLX_VENDOR_ID);
	 table->subtype = htons(FSLXIT_WRITE_TABLE_ACTIONS);
	 table->table_id = htonl(table_id);
...
#endif
int32_t fslx_instruction_apply_table_actions(struct of_msg *msg, uint8_t table_id)
{
	 struct of_msg_descriptor *msg_desc;
	 struct fslxit_table_actions *table;
	 ...
	 if ((msg_desc->ptr2+msg_desc->length2 + FSLXIT_APPLY_TABLE_ACTIONS_LEN) >
				 (msg->desc.start_of_data + msg_desc->buffer_len)) 
	 {
			OF_LOG_MSG(OF_LOG_UTIL_L3EXT, OF_LOG_WARN,
						"Length of buffer is not sufficient to add data");
	 ...
	 table = (struct fslxit_table_actions*)(msg_desc->ptr2);
	 table->type = htons(OFPIT_EXPERIMENTER);
	 table->len  = htons(FSLXIT_APPLY_TABLE_ACTIONS_LEN);
	 table->vendor = htonl(FSLX_VENDOR_ID);
	 table->subtype = htons(FSLXIT_APPLY_TABLE_ACTIONS);
         table->table_id= table_id;

	 msg_desc->length2 += FSLXIT_APPLY_TABLE_ACTIONS_LEN;
	 msg_desc->length3  = 0;
	 msg->desc.data_len += msg->desc.length2;
...
int32_t
fslx_action_bind_mod(struct of_msg *msg,
                     uint32_t bind_obj_id, 
                     uint8_t app_type, 
                     uint8_t obj_type,
                     uint8_t command, 
...
{
  struct of_msg_descriptor *msg_desc;
  struct fslxat_bind_mod *bindmod_action;
  struct fslx_bind_mod *bind_mod; 
  uint32_t action_len;
  ...
  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "bindobj-id=%d inst=%p instlen=%d ",bind_obj_id, instructions, inst_len);


  action_len = (sizeof(struct fslxat_bind_mod) + inst_len);
  ...
  bindmod_action = (struct fslxat_bind_mod *)(msg_desc->ptr3+msg_desc->length3);
  bindmod_action->type       = htons(OFPAT_EXPERIMENTER);
  bindmod_action->len        = htons(action_len);
  bindmod_action->vendor     = htonl(FSLX_VENDOR_ID);
  bindmod_action->subtype    = htons(FSLXAT_BIND_MOD);
  bindmod_action->hit_count  = 0;

  /* fill ofp header */
#if 0
  fslx_header = (struct fslx_header *)(msg_desc->ptr3+msg_desc->length3);
  fslx_header->header.version = OFP_VERSION;
  fslx_header->header.type    = OFPT_EXPERIMENTER;
  fslx_header->header.length  = htons((sizeof(struct fslxat_bind_mod) + inst_len));
  fslx_header->header.xid  = htonl(0x5678);
  fslx_header->subtype     = htonl(FSLX_BIND_MOD)
  fslx_header->vendor     = htonl(FSLX_VENDOR_ID)
#endif

  /* fill bind-mod fixed information */
  bind_mod = (struct fslx_bind_mod *)(&(bindmod_action->bind_mod));
  bind_mod->bind_obj_id = htonl(bind_obj_id);
  bind_mod->type                       = app_type;
  bind_mod->obj_type                   = obj_type;
  bind_mod->command                    = command;
  bind_mod->remove_flow_config         = remove_flow_config; 
  bind_mod->del_bind_obj_config        = del_bind_obj_config;
  bind_mod->cookie                     = htonll(cookie);
  bind_mod->life_time_as_timeout       = htonll(life_time_as_timeout);
  bind_mod->life_time_as_no_of_bytes   = htonll(life_time_as_no_of_bytes);
  bind_mod->life_time_as_no_of_packets = htonll(life_time_as_no_of_packets);
  bind_mod->idle_timeout               = htonll(idle_timeout);

  /* fill instructions */
  if (inst_len)
      memcpy(bind_mod->instructions, instructions, inst_len);
