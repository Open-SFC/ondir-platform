...
extern int mempool_create_mheap_pool (struct mempool_params *pool_params_p,
      void*    mempool_p);
...
{
   OF_LOG_MSG(OF_LOG_UTIL, OF_LOG_DEBUG,"UnLoading MHeap Infra Library");

   do
   {
#if 0
      ret_val=of_mheap_deinit();
      if(ret_val != OF_SUCCESS)
      {
         OF_LOG_MSG(OF_LOG_UTIL, OF_LOG_DEBUG, "MHeap Infra initialization failed");
         status= OF_FAILURE;
