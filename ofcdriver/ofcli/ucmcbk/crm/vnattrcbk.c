...

{
  struct   cm_array_of_iv_pairs *result_iv_pairs_p = NULL;
  int32_t  return_value = OF_FAILURE;
  struct   crm_attribute_name_value_output_info crm_attribute_name_value_info={};
  struct   crm_attribute_name_value_pair  crm_attribute_name_value_pair = {};
  char     vn_name[CRM_MAX_VN_NAME_LEN+1]={};
  uint64_t vn_handle;
  ...
  if ((crm_vn_attribute_ucm_setmandparams (keys_arr_p,vn_name, &crm_attribute_name_value_pair, 
	      NULL)) != OF_SUCCESS)
   {
     CM_CBK_DEBUG_PRINT ("Set Mandatory Parameters Failed");
     return OF_FAILURE;
  ...
  return_value = crm_get_vn_exact_attribute(vn_handle, crm_attribute_name_value_info.name_string,&crm_attribute_name_value_info);
    if (return_value != OF_SUCCESS)
     {
