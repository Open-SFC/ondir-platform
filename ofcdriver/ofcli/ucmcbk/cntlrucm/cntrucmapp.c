...
void cntrl_ucm_process_request (struct sockaddr_in *peer_address, ucmcntrlmsg_t  *ucm_request)
{
  ...
  uint32_t trans_cmd;
  int32_t cmd_id,result=0,ii=0;
  char *msg_buf;
  uint32_t no_of_records=0,rec_cnt;
...
#else
	 msg_buf= of_mbuf_put_32(msg_buf, (uint32_t) config_trans_p);
#endif
...
void cntrl_ucm_send_multipart_response(uint32_t multipart_type,void *data_p, uint8_t last)
{
  ...
  uint32_t total_length=0, no_of_records=1, length, rec_index=0,ii;
  char *msg_buf;
  struct cm_array_of_iv_pairs *result_iv_pairs_p=NULL;
  ...
  switch(multipart_type)
  {
    ...
    case OFPMP_GROUP_DESC:
      ...
      break;
    case OFPMP_QUEUE:
      //cntrl_response->app_id=CM_ON_DIRECTOR_DATAPATH_STATS_QUEUESTATS_APPL_ID;
      break;
    case OFPMP_GROUP:
    ...
    case OFPT_GET_ASYNC_REPLY:
      cntrl_response->app_id=CM_ON_DIRECTOR_DATAPATH_ASYNCMSGCFG_APPL_ID;
      break;
    case OFPMP_EXPERIMENTER:
      cntrl_response->app_id=CM_ON_DIRECTOR_DATAPATH_STATS_BINDSTATS_APPL_ID;
      break;
   case OFPT_QUEUE_GET_CONFIG_REPLY:
       cntrl_response->app_id=CM_ON_DIRECTOR_DATAPATH_QUEUE_CONFIG_APPL_ID;
        break;
    default:
  ...
  switch(cntrl_response->app_id)
  {
   ...
   case CM_ON_DIRECTOR_DATAPATH_QUEUE_CONFIG_APPL_ID:
     {
          struct ofl_queue_config *queue_config  = (struct ofl_queue_config *)data_p;
           cntrlrucm_debugmsg("%s(%d):: openflow queue config response",__FUNCTION__,__LINE__);
            of_queue_ucm_getparams(queue_config,&result_iv_pairs_p[rec_index]);
           break;
      }
    case CM_ON_DIRECTOR_DATAPATH_ASYNCMSGCFG_APPL_ID:
    {
        struct ofp_async_config   *of_async_config_msg = (struct ofp_async_config *)data_p;
        cntrlrucm_debugmsg("%s(%d):: async reply response",__FUNCTION__,__LINE__);
	dprm_datapath_asyncmsgcfg_ucm_getparms(of_async_config_msg, &result_iv_pairs_p[rec_index]);
        break;
      }
