...
struct cm_dm_app_callbacks of_ssl_addnlca_ucm_callbacks = {
       ...
       NULL,
       NULL,
       NULL, //of_ssl_addnlcacertsverifycfg,
       NULL
};
...
int32_t of_ssl_appl_addnlcacerts_ucmcbk_init (void)
{
  ...
  cm_register_app_callbacks (CM_ON_DIRECTOR_SSL_ADNLCA_APPL_ID, &of_ssl_addnlca_ucm_callbacks); 
  return OF_SUCCESS;
}
...
//    FillAppResultStruct (&of_domain_result, NULL, CM_GLU_VLAN_FAILED);
  ...
  return_value = of_ssl_load_ca_certificate((struct cntlr_ucm_cacert_info *) &cert_info);
  if (return_value != OF_SUCCESS)
  {
