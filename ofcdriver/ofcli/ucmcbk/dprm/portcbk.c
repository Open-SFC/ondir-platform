...
int32_t dprm_datapath_port_modrec (void * config_trans_p,
...
struct cm_dm_app_callbacks dprm_datapath_port_ucm_callbacks = {
	 ...
	 dprm_datapath_port_modrec,
	 dprm_datapath_port_delrec,
	 NULL,
			...
			struct cm_app_result ** result_p)
{
  ...
  of_memset (&port_info, 0, sizeof (struct ofp_port_mod));
  ...
  return_value = dprm_get_port_info_by_port_id(datapath_handle , port_info.port_no, &port_info);
  if(DPRM_SUCCESS != return_value)
  {
    CM_CBK_DEBUG_PRINT ("Failed to get port handle");
    return OF_FAILURE;
  }
    ...
    struct ofp_port_mod* port_info,		struct cm_app_result ** presult_p)
{
      ...
      uiMandParamCnt++)
  {
	...
	port_info->port_no=iportId;
        CM_CBK_DEBUG_PRINT ("port ID %d",  port_info->port_no);
	break;
    }
  }
  CM_CBK_PRINT_IVPAIR_ARRAY (pMandParams);
    ...
    struct dprm_port_info* port_info,		struct cm_app_result ** presult_p)
{
      ...
      uiMandParamCnt++)
  {
	...
	if(pMandParams->iv_pairs[uiMandParamCnt].value_length > DPRM_MAX_PORT_NAME_LEN)
    {
	port_info->port_name[DPRM_MAX_PORT_NAME_LEN] = '\0';
	CM_CBK_DEBUG_PRINT ("ERROR:Buffer overflow");
	}
	CM_CBK_DEBUG_PRINT ("port name %s",port_info->port_name);
	break;
	...
	port_info->port_id=iportId;
        CM_CBK_DEBUG_PRINT ("port ID %d",  port_info->port_id);
	break;
    }
  }
  CM_CBK_PRINT_IVPAIR_ARRAY (pMandParams);
...
#define CM_PORT_CHILD_COUNT 11
  ...
  else
  {
    ...
    { 
      ...
      {
	if(strlen(ofp_port_config[ii]) < 64 )
  ...
  else
  {
    for(ii=0;ii<3;ii++)
    {
      ...
      {
	if(strlen(ofp_port_state[ii]) < 64 )
  ...
  else
  {
    for (ii=0; ii < 16; ii++ )
    {
      ...
      {
	if(strlen(fp_port_features[ii]) < 64 )
  ...
  memset(buf, 0, sizeof(buf));
  if(port_info->port_desc.advertised == 0)
  {
    strcpy(buf,"none");
  }
  else
  {
    for (ii=0; ii < 16; ii++ )
    {
      if ( port_info->port_desc.advertised & (1 << ii) )
      {
	if(strlen(fp_port_features[ii]) < 64 )
	strcat(buf, fp_port_features[ii]);	
	else 
  ...
  memset(buf, 0, sizeof(buf));
  if(port_info->port_desc.supported == 0)
  {
    strcpy(buf,"none");
  }
  else
  {
    for (ii=0; ii < 16; ii++ )
    {
      if ( port_info->port_desc.supported & (1 << ii) )
      {
	if(strlen(fp_port_features[ii]) < 64 )
	strcat(buf, fp_port_features[ii]);	
	else 
  ...
  else
  {
    for (ii=0; ii < 16; ii++ )
    {
      if ( port_info->port_desc.peer & (1 << ii) )
      {
	if(strlen(fp_port_features[ii]) < 64 )
	strcat(buf, fp_port_features[ii]);	
	else 
