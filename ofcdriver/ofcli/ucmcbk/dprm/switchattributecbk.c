                        ...
                        struct cm_array_of_iv_pairs ** next_n_record_data_p)
{
	...
	struct   dprm_switch_info            switch_info={};
	char     current_name_string[DPRM_MAX_ATTRIBUTE_NAME_LEN+1];
	char     current_value_string[DPRM_MAX_ATTRIBUTE_VALUE_LEN+1];
	...
	strncpy(current_name_string,  prev_record_key_p->iv_pairs[uindex_i].value_p , DPRM_MAX_ATTRIBUTE_VALUE_LEN);
	if(prev_record_key_p->iv_pairs[uindex_i].value_length > DPRM_MAX_ATTRIBUTE_VALUE_LEN)
	{
		current_name_string[DPRM_MAX_ATTRIBUTE_VALUE_LEN] = '\0';
		CM_CBK_DEBUG_PRINT ("Buffer Overflow");
		return OF_FAILURE;
	}
	//strcpy(current_value_string, prev_record_key_p->iv_pairs[uindex_i+1].value_p);
	strncpy(current_value_string,  prev_record_key_p->iv_pairs[uindex_i+1].value_p , DPRM_MAX_ATTRIBUTE_VALUE_LEN);
	if(prev_record_key_p->iv_pairs[uindex_i+1].value_length > DPRM_MAX_ATTRIBUTE_VALUE_LEN)
	{
		current_name_string[DPRM_MAX_ATTRIBUTE_VALUE_LEN] = '\0';
		CM_CBK_DEBUG_PRINT ("Buffer Overflow");
		return OF_FAILURE;
...

{
	 ...
	 if (return_value != OF_SUCCESS)
	 {
         ...
         dprm_switch_attribute_getparams (&dprm_attribute_output, result_iv_pairs_p);
			...
			switch (pMandParams->iv_pairs[uiMandParamCnt].id_ui)
			{
				 case CM_DM_ONSWITCH_NAME_ID:
                                                of_strncpy (of_switch->name,
                                                                        ...
                                                                        DPRM_MAX_SWITCH_NAME_LEN);/*pMandParams->iv_pairs[uiMandParamCnt].value_length);*/
						if(pMandParams->iv_pairs[uiMandParamCnt].value_length > DPRM_MAX_SWITCH_NAME_LEN)							{
								of_switch->name[DPRM_MAX_SWITCH_NAME_LEN] = '\0';
