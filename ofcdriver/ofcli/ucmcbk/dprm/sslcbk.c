...
int32_t of_ssl_appl_ucmcbk_init (void)
{
  ...
  cm_register_app_callbacks (CM_ON_DIRECTOR_SSL_APPL_ID, &of_ssl_ucm_callbacks);
  return OF_SUCCESS;
}
    ...
    switch (pOptParams->iv_pairs[ii].id_ui)
    {
      case CM_DM_SSL_CACERT_ID:
      {
        of_strncpy(of_certs->ca_cert_file,
                   ...
                   CNTLR_CM_CERT_FILE_PATH_SIZE - 1);
		if(pOptParams->iv_pairs[ii].value_length >= CNTLR_CM_CERT_FILE_PATH_SIZE)
  		{
			of_certs->ca_cert_file[CNTLR_CM_CERT_FILE_PATH_SIZE - 1] = '\0';
  			CM_CBK_DEBUG_PRINT ("ERROR:Buffer oveflow");
		}
        break;
      }
