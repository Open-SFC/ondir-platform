		...
		struct cm_app_result ** result_p)
{
  ...
  if ((dprm_datapath_asyncmsgcfg_ucm_setmandparams (pMandParams, &datapath_info, NULL)) != OF_SUCCESS)
  {
  ...
  if (iRetVal != OF_SUCCESS)
  {
    ...
    *result_p = async_msg_cfg_result;
	...
	if((controller_role == OFPCR_ROLE_SLAVE))
	{
       ...
       if(dprm_get_async_cfg(datapath_handle, &of_async_config_msg) != DPRM_SUCCESS)
       {
         OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,
                "Error in getting async msg info");
         return OF_FAILURE;
       }
  if((dprm_datapath_asyncmsgcfg_ucm_setoptparams(pOptParams,&of_async_config_msg, NULL))!=OF_SUCCESS)
	...
	if(iRetVal != CNTLR_SUCCESS) 
	{
       ...
       if(dprm_set_async_cfg_ctl_db(datapath_handle, &of_async_config_msg) != DPRM_SUCCESS)
       {
         OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,
                "\nError in setting async msg info in ctl local database\n\r");
         return OF_FAILURE;
       }
...
//	if(of_atoi((char *)(pOptsParams->iv_pairs[uiOptsParamCnt].value_p)) == TRUE)
	if(strcmp(((char *)(pOptsParams->iv_pairs[uiOptsParamCnt].value_p)), "enable") == 0)
	{
    ...
    else if(strcmp(((char *)(pOptsParams->iv_pairs[uiOptsParamCnt].value_p)), "disable") == 0)
    {
      switch (pOptsParams->iv_pairs[uiOptsParamCnt].id_ui)
	  {
			/* Packet-In Async Config messages */
...
#define CM_AYSNCMSGCFG_CHILD_COUNT 20
	...
	strcpy(string, "No matching flow for master enabled.");
	else
	strcpy(string, "No matching flow for master disabled.");	
	...
	strcpy(string, "No matching flow for slave enabled.");
	else
	strcpy(string, "No matching flow for slave disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "Action explicitly output for master enabled.");
	else
	strcpy(string, "Action explicitly output for master disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "Action explicitly output for slave enabled.");
	else
	strcpy(string, "Action explicitly output for slave disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	 CM_DM_ASYNCMSGCFG_SLAVE_PACKET_IN_ACTION_ID,CM_DATA_TYPE_STRING, string);
	index++;
	...
	strcpy(string, "Port was added async event enabled.");
	else
	strcpy(string, "Port was added async event disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "Port was removed async event enabled.");
	else
	strcpy(string, "Port was removed async event disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "Port was modify async event enabled.");
	else
	strcpy(string, "Port was modify async event disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "Flow idle time exceeded,async event enabled");
	else
	strcpy(string, "Flow idle time exceeded,async event disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "Time exceeded hard_timeout async event enabled");
	else
	strcpy(string, "Time exceeded hard_timeout async event disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "DELETE flow mod async event enabled");
	else
	strcpy(string, "DELETE flow mod async event disabled");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "Group was removed async event enabled");
	else
	strcpy(string, "Group was removed async event disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	...
	strcpy(string, "Group was removed async event disabled.");	
	FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],
	 CM_DM_ASYNCMSGCFG_SLAVE_FLOW_REMOVED_GROUP_DELETE_ID, CM_DATA_TYPE_STRING, string);
	index++;
