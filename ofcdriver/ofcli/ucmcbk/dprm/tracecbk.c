...
uint8_t of_ipsec_redside_frag_g = 0;
    ...
    struct cm_app_result ** result_p)
{
      ...
      param_count++)
  {
    switch (opt_iv_pairs_p->iv_pairs[param_count].id_ui)
    {
      case CM_DM_TRACE_MODULE_ID:
	...
	else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "cntlrxprt")==0)
        {
        ...
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "arpapp")==0)
        {
          trace_module = OF_LOG_ARP_APP;
        }
        ...
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "ipv4fwd")==0)
        {
          trace_module = OF_LOG_IP4_FWD;
        }
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "ipv4mcastfwd")==0)
        {
          trace_module = OF_LOG_IP4_MCFWD;
        }
        ...
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "fwnat")==0)
        {
          trace_module = OF_LOG_FWNAT;
        }
        ...
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "bindobj")==0)
        {
          trace_module = OF_LOG_BIND_OBJ;
        }
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "l3_sub_ttp")==0)
        {
          trace_module = OF_LOG_L3_SUB_TTP;
        }
	else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "arpsubttp")==0)
        {
          trace_module = OF_LOG_ARP_SUB_TTP;
        }
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "qosapp")==0)
        {
          trace_module = OF_LOG_QOS_APP;
        }
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "qosnf")==0)
        {
          trace_module = OF_LOG_QOS_NF;
        }
        else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "qos")==0)
        {
          trace_module = OF_LOG_QOS;
        }
       else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "qossubttp")==0)
        {
          trace_module = OF_LOG_QOS_SUB_TTP;
        }
        ...
        case CM_DM_TRACE_IPSEC_REDSIDE_FRAG_ID:
        {
          if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "enable")==0)
          {
            of_ipsec_redside_frag_g = 1;
          }
          else if(strcmp(opt_iv_pairs_p->iv_pairs[param_count].value_p, "disable")==0)
          {
            of_ipsec_redside_frag_g = 0;
          }
          else 
          {
            CM_CBK_DEBUG_PRINT ("Invalid option (%s)",opt_iv_pairs_p->iv_pairs[param_count].value_p);
            return OF_FAILURE;
          }
          return OF_SUCCESS;
        }
    ...
    if(trace_params_values[ii].is_trace_module_set == TRACE_MODULE_SET){
      switch(ii){
        case  OF_LOG_MOD:
          ...
          break;
        case OF_LOG_XPRT_PROTO:
          {
            strcat(buf, "\tOF_LOG_XPRT_PROTO : ");
          }
          break;
        case OF_NEM_LOG:
          {
        ...
        case OF_LOG_IP4_FWD:
          {
            strcat(buf, "\tOF_LOG_IP4_FWD : ");
          }
          break;
        case OF_LOG_IP4_MCFWD:
          {
            strcat(buf, "\tOF_LOG_IP4_MCFWD : ");
          }
          break;
        case OF_LOG_IPSEC:
          {
	 ...
	 break;
        ...
        case OF_LOG_TSC:
          ...
          break;
        case OF_LOG_BIND_OBJ:
          {
            strcat(buf, "\tOF_LOG_BIND_OBJ : ");
          }
          break;
        case OF_LOG_ARP_SUB_TTP:
          {
            strcat(buf, "\tOF_LOG_TTP : ");
          }
          break;
 
        case OF_LOG_L3_SUB_TTP:
          {
            strcat(buf, "\tOF_LOG_L3_SUB_TTP : ");
          }
          break;
        case OF_LOG_QOS_APP:
          {
            strcat(buf, "\tOF_LOG_QOS_APP : ");
          }
          break;
        case OF_LOG_QOS_NF:
          {
            strcat(buf, "\tOF_LOG_QOS_NF : ");
          }
          break;
        case OF_LOG_QOS:
          {
            strcat(buf, "\tOF_LOG_QOS : ");
          }
          break;
       case OF_LOG_QOS_SUB_TTP:
          {
            strcat(buf, "\tOF_LOG_QOS_SUB_TTP : ");
          }
          break;
