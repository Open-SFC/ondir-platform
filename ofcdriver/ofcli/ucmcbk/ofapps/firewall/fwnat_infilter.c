...
/*
 *
 * File name: fwnat_infilter.c
 * Author: Jothirlatha J <b37228@freescale.com> 
 * Date:   10/13/2014
 * Description: 
 * 
 */
/* File  :      fwnat_infilter.c
...
//#define FWNAT_CM_DEBUG(a,b...)   printf(a,b);
#define FWNAT_CM_DEBUG(a,b...) 
...
struct cm_dm_app_callbacks of_fwnat_infilter_callbacks =
{
   ...
   NULL,
   NULL,
   of_fwnat_infilter_getfirstrec,
   of_fwnat_infilter_getnextrec,
   of_fwnat_infilter_getexactrec,
   NULL,
   NULL
}; 

int32_t of_fwnat_infilter_appl_ucmcbk_init()
{
   CM_CBK_DEBUG_PRINT ("FWNAT - input filter Entry");
   cm_register_app_callbacks (CM_ON_DIRECTOR_DATAPATH_FIREWALL_FILTERINPUT_APPL_ID,
                  &of_fwnat_infilter_callbacks);
   return OF_SUCCESS;
}
int32_t of_fwnat_infilter_getfirstrec 
     (struct cm_array_of_iv_pairs * key_iv_pairs_p,
      ...
      struct cm_array_of_iv_pairs ** array_of_iv_pair_arr_p)
{
  ...
  *count_p = 0;
  ...
  status = of_fwnat_infilter_ucm_setmandparams (&fwnat_inargs, 
                               key_iv_pairs_p, 
  ...
  /* This is the funtion call for input filter chain. 
   * Set the chain name. 
   */ 
  strcpy (fwnat_inargs.chainname,"INPUT"); 
  ...
  status = fwnat_infilter_ucm_getparams (&fwnat_inargs, 
                                 result_iv_pairs_p); 
  if (status != OF_SUCCESS)
  {
		CM_CBK_DEBUG_PRINT ("fwnat_infilter_ucm_getparams failed");
    return OF_FAILURE; 
  }
...
#if 0
void PrintCmFilterArgs (struct of_fwnat_cm_filter_inargs *inargs)
{
  FWNAT_CM_DEBUG ("\n\r%s:%s:%d  Entered. \n\r", 
          __FILE__, __FUNCTION__, __LINE__); 
  FWNAT_CM_DEBUG("idpID = %llx \n\r",inargs->idpID); 
  FWNAT_CM_DEBUG("tablename = %s \n\r",inargs->tablename); 
  FWNAT_CM_DEBUG("chainname = %s \n\r",inargs->chainname); 
  FWNAT_CM_DEBUG("inifacename = %s \n\r",inargs->inifacename); 
  FWNAT_CM_DEBUG("outifacename = %s \n\r",inargs->outifacename); 
  FWNAT_CM_DEBUG("sourceip = %s \n\r",inargs->sourceip); 
  FWNAT_CM_DEBUG("destip = %s \n\r",inargs->destip); 
  FWNAT_CM_DEBUG("sourceport = %s \n\r",inargs->sourceport); 
  FWNAT_CM_DEBUG("destport = %s \n\r",inargs->destport); 
  FWNAT_CM_DEBUG("protocol = %s \n\r",inargs->protocol); 
  FWNAT_CM_DEBUG("action = %s \n\r",inargs->action); 
  FWNAT_CM_DEBUG("position = %s \n\r",inargs->position); 
  FWNAT_CM_DEBUG ("%s:%s:%d  Exited. \n\r", 
          __FILE__, __FUNCTION__, __LINE__); 
  return;
}
#endif 
...
#define CM_FILTERINPUT_CHILD_COUNT  13
  result_iv_pairs_p->iv_pairs =
    (struct cm_iv_pair *) of_calloc (CM_FILTERINPUT_CHILD_COUNT, 
  ...
  /* ns name */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_TABLENAME_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->nsname);
  iCount++; 
#endif 
  ...
  /* 3. Chain name */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_CHAINNAME_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->chainname);
  iCount++; 
  /* 4. source ip */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_SOURCEIP_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->sourceip);
  iCount++; 
  /* 5 Dest ip */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_DESTIP_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->destip);
  iCount++; 
  /* 6 source Port */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_SOURCEPORT_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->sourceport);
  iCount++; 
  /* 7 Dest Port */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_DESTPORT_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->destport);
  iCount++; 
  /* 8 Protocol */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_PROTOCOL_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->protocol);
  iCount++; 
  /*  9 Input interface Name  */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_INIFACENAME_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->inifacename);
  iCount++; 
  /* 10 Output interface Name  */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_OUTIFACENAME_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->outifacename);
  iCount++; 
  /* 12 rule Position */ 
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_RULEPOSITION_ID,
      CM_DATA_TYPE_STRING, 
  ...
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_FLOW_PRIORITY_ID,
      CM_DATA_TYPE_STRING, string);
  iCount++; 

  /* 11 Action */
  FILL_CM_IV_PAIR (result_iv_pairs_p->iv_pairs[iCount],
      CM_DM_FILTERINPUT_ACTION_ID,
      CM_DATA_TYPE_STRING, 
      fwnat_inargs->action);
  iCount++; 
      ...
      struct cm_array_of_iv_pairs ** next_n_record_data_p)
{
  ...
  if (OF_SUCCESS != of_fwnat_infilter_ucm_setmandparams(
  ...
  if (OF_SUCCESS !=  of_fwnat_infilter_ucm_setoptparams 
                 (prev_record_key_p,
                  &fwnat_inargs,
                  &app_result_p))
  {
         ...
         case CM_DM_DATAPATH_DATAPATHID_ID:
           {
            ...
            break;
         case CM_DM_FILTERINPUT_TABLENAME_ID:
           {
...
int32_t of_fwnat_infilter_ucm_setoptparams (struct cm_array_of_iv_pairs *
      opt_iv_pairs_p,
      ...
      struct cm_app_result ** presult_p)
{
  ...
  FWNAT_CM_DEBUG ("Entered. iv pair count = %u ", 
  ...
  for (count = 0; count < opt_iv_pairs_p->count_ui; count++)
  {
    ...
    {
      case CM_DM_FILTERINPUT_RULEPOSITION_ID:
  ...
  strcpy(fwnat_cm_filter_inargs->chainname,"INPUT"); 
