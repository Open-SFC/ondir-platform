...
int32_t UCMSendCmdToController( ucmcommmsg_t *ucm_comm_msg_p, int32_t *pOutLen)
{
	 ...
	 do
	 {
			if(PollFds[0].returned_events & CM_FDPOLL_READ)
			{
				 ...
				 return_value = cm_socket_recv_from(iCntrSockfd_g, 
                                                 (char*)&CntrlResp, 
                                                 sizeof(ucmcntrlmsg_t), 0, &peer_ip, &PeerPort);
				 if(return_value < 0)
				 {
				 ...
				 if(CntrlResp.msg_type == CM_CNTRL_MULTIPART_RESPONSE)
				 {
						...
						if((CntrlResp.length <= (CM_CNTRL_MAX_MULTIPART_RESP_LEN - ulMultiPartBufLen))
						&& (!pMultiPartBuf))
