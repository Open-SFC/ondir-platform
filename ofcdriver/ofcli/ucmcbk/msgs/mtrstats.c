      ...
      mand_param_cnt++)
  {
    switch (mand_iv_pairs->iv_pairs[mand_param_cnt].id_ui)
    {
      case CM_DM_METERS_METER_ID:
        of_atoi((char *) mand_iv_pairs->iv_pairs[mand_param_cnt].value_p);
        break;
    }
  }
  CM_CBK_PRINT_IVPAIR_ARRAY (mand_iv_pairs);*/
    ...
    struct cm_array_of_iv_pairs * result_iv_pairs_p)
{
  ...
  struct ofi_meter_band_stats *band_entry_p;

#define CM_METERSTATS_CHILD_COUNT 15
  ...
  OF_LIST_SCAN(meter_stats_p->meter_band_stats, band_entry_p, struct ofi_meter_band_stats *,OF_METER_BAND_STATS_LISTNODE_OFFSET)
  {
    of_memset(string, 0, sizeof(string));
    of_sprintf(string,"packet_band_count=%llu byte_band_count=%llu",band_entry_p->packet_count,band_entry_p->byte_count);
    FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],CM_DM_METERSTATS_BANDSTATS_ID,CM_DATA_TYPE_STRING, string);
    index++;
  }
