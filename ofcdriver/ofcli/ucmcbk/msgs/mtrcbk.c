    ...
    struct cm_array_of_iv_pairs ** arr_ivpair_arr_pp)
{
  ...
  return_value=cntlr_send_meter_config_request(datapath_handle,OFPM_ALL);
  if ( return_value != OF_SUCCESS)
  {
...
#define CM_METERREC_CHILD_COUNT 20
  ...
  of_memset(string, 0, sizeof(string));
  if(meter_entry_p->flags & OFPMF_KBPS)
     of_sprintf(string,"%s","OFPMF_KBPS");
  else if(meter_entry_p->flags & OFPMF_PKTPS)
     of_sprintf(string,"%s","OFPMF_PKTPS");
  else if(meter_entry_p->flags & OFPMF_BURST)
     of_sprintf(string,"%s","OFPMF_BURST");
  ...
  {
    of_memset(string, 0, sizeof(string));
    of_memset(string1, 0, sizeof(string1));
	...
	of_sprintf(string1,"%s","OFPMBT_DROP");
	...
	of_sprintf(string1,"%s","OFPMBT_DSCP_REMARK");
        of_sprintf(string,"Length=%d  Rate=%d burst_size=%d prec_level=%d",len,rate,burst_size,prec_level);
        break;
      case OFPMBT_EXPERIMENTER:
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"not supported");
	 of_sprintf(string1,"%s","OFPMBT_EXPERIMENTER");
        of_strcpy(string,"-");
        break;
    }
    FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],CM_DM_METERREC_BANDTYPE_ID,CM_DATA_TYPE_STRING, string1);
    index++;
