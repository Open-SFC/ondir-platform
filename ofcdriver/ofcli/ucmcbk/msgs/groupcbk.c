          ...
          case OFPAT_SET_FIELD:
            {
              ...
              switch(action_entry_p->setfield_type){
                ...
                case OFPXMT_OFB_IPV4_SRC:
                  ...
                  of_sprintf(tmp_string," IPv4 source address:= %x",buf);
                  break;
                case OFPXMT_OFB_IPV4_DST:
                  of_memset(buf, 0, sizeof(buf));
                  cm_inet_ntoal(action_entry_p->ui32_data, buf);
                  of_sprintf(tmp_string," IPv4 destination address:= %x",buf);
                  break;
                ...
                case OFPXMT_OFB_ARP_SPA:
                  of_memset(buf, 0, sizeof(buf));
                  cm_inet_ntoal(action_entry_p->ui32_data, buf);
                  of_sprintf(tmp_string," ARP source IPv4 address:= %x",buf);
                  break;
                case OFPXMT_OFB_ARP_TPA:
                  of_memset(buf, 0, sizeof(buf));
                  cm_inet_ntoal(action_entry_p->ui32_data, buf);
                  of_sprintf(tmp_string," ARP target IPv4 address:= %x",buf);
                  break;
