...
int32_t fwconn_ttp_stats_appl_init (void);

int32_t fwconn_ttp_stats_getexactrec (struct cm_array_of_iv_pairs * pKeysArr,
    struct cm_array_of_iv_pairs ** result_iv_pairs_pp);

int32_t fwconn_ttp_stats_getparams (struct fw4_conntrack_subttp_stats *stats, struct cm_array_of_iv_pairs * result_iv_pairs_p);

struct cm_dm_app_callbacks fwconn_ttp_stats_callbacks = 
{
  NULL,
  NULL,
  NULL,
...
int32_t fwconn_ttp_stats_appl_init (void)
{
  ...
  return_value= cm_register_app_callbacks (CM_ON_DIRECTOR_DATAPATH_SUBTTP_FIREWALL_STATS_FWCONSTATS_APPL_ID,
      &fwconn_ttp_stats_callbacks);
  if(return_value != OF_SUCCESS)
  {
    CM_CBK_DEBUG_PRINT("fw connection track sub ttp stats CBK registration failed");
    return OF_FAILURE;
  }
    ...
    struct cm_array_of_iv_pairs ** result_iv_pairs_pp)
{
  ...
  struct cm_app_result *fwconn_ttp_stats_result = NULL;
  uint32_t  ns_id, rec_count=0;
  struct cm_array_of_iv_pairs *result_iv_pairs_p=NULL;
  struct fw4_conntrack_subttp_stats fwconn_ttp_stats;

  if ((fwconn_ttp_stats_setmandparams (pKeysArr, &ns_id, &fwconn_ttp_stats_result)) !=
      OF_SUCCESS)
  {
  ...
  return_value = get_fw4_conntrack_subttp_stats(0, &fwconn_ttp_stats) ;
  if (return_value == OF_SUCCESS)
  {
    result_iv_pairs_p =
    ...
    fwconn_ttp_stats_getparams(&fwconn_ttp_stats, &result_iv_pairs_p[rec_count]); 
    rec_count ++;
    *result_iv_pairs_pp = result_iv_pairs_p;
    CM_CBK_DEBUG_PRINT ("Firewall connection track ttp stats response");
    return OF_SUCCESS;
  }
  *result_iv_pairs_pp = result_iv_pairs_p;
  CM_CBK_DEBUG_PRINT ("Firewall connection track ttp stats response failed");
  return OF_FAILURE;
}

int32_t fwconn_ttp_stats_setmandparams (struct cm_array_of_iv_pairs *pMandParams,	
...
#define CM_FWCONNSTATS_CHILD_COUNT 22
