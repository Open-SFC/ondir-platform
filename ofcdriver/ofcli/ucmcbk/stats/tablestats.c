...
#define CM_TABLESTATS_CHILD_COUNT 6
  ...
  of_memset(string, 0, sizeof(string));
  of_ltoa (table_info_p->total_flows_received, string);
  FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],CM_DM_TABLESTATS_TOTAL_FLOWS_RECEIVED_ID,CM_DATA_TYPE_STRING, string);
  index++;

  of_memset(string, 0, sizeof(string));
  of_ltoa (table_info_p->flow_failed_count, string);
  FILL_CM_IV_PAIR(result_iv_pairs_p->iv_pairs[index],CM_DM_TABLESTATS_FLOW_FAIL_COUNT_ID, CM_DATA_TYPE_STRING, string);
  index++;
