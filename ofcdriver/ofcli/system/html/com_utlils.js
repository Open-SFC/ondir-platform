...
function setHostID(vHost) { defaultHostType="SingleDevice"; return; //SDN
...
function updateHostInfo() {
  var pdoc = parent.parent.frameTop.tBar.document
  if(pdoc != null) {
    updateToolBar(pdoc);
  }
}
...
function setProductName(pDoc) {
   pDoc.getElementById("productNameDiv").innerHTML="<font face=\"Verdana, Arial, Helvetica, sans-serif\" size=\"4\" color=\"#fff\"><i>" + productName + "&#160;</i></font>";   
}
function setProductNameInBlackFont(pDoc) {
   pDoc.getElementById("productNameDiv").innerHTML="<font face=\"Verdana, Arial, Helvetica, sans-serif\" size=\"4\" color=\"#000\"><i>" + productName + "&#160;</i></font>";
}
function setProductNameAsTitle(pDoc) {   
   var titleObj = pDoc.getElementsByTagName("title");
   var IE = document.all?true:false;
   if (!IE) {
      titleObj[0].innerHTML = productName;
   }
}
function getHostID() {
  return "localhost"; //SDN
  /*var hostID="";
  ...
  for(i=0; i<mySplitResult.length; i++) {
    ...
    switch(splRes[0]) {
      ...
      case "vrtInstName":
        ...
        break;
      case "host":
        host = "localhost"; //splRes[1]; //SDN
        //alert("host= " + host);
        if(host == "")
         host = "localhost";//defaultHost; //SDN
        break;
      } // switch 
  } // for
  ...
  if (typeof XMLHttpRequest == 'undefined') {
     objXmlHttp = function() {
        ...
        for (var i = 0; i < xhrList.length; ++i) {
            try {
                ieXHR = new ActiveXObject(xhrList[i]);
                return ieXHR;
            }
            catch (e) {}
        }
        alert("Error! Scripting for ActiveX might be disabled.");
        return undefined; //leave it undefined for when it doesn't work
    };
  } else {
    objXmlHttp = new XMLHttpRequest(); //common for all
...
function sendRequestToServer(urlStr, method, params, respHandler) {
  ...
  //xmlobject.setRequestHeader("If-Modified-Since", "Sat, 1 Jan 2000 00:00:00 GMT");
  //try {xmlobject.responseType = "msxml-document"} catch(err) {} // Helping IE11
  xmlobject.send(params);
}
function sendRequestToServerEx(urlStr, method, params, respHandler, tvar) {
  /*
   * NOTE: POST method is prefered to overcome
   * GET URL length limitation on the server.
   */
               ...
               if (depwhere != null && depwhere != '') {
                if (depwhereMatched) {
                 ...
                 if(curVal != null && curVal == depValueStr) {                  
                  selObj.options[selObj.options.length] = 
                      new Option(depValueStr, depValueStr, 'TRUE');                 
                  selIndex = selObj.options.length - 1; //alert(selIndex); 
                  selObj.selectedIndex = selIndex; 
                 } else {
                  selObj.options[selObj.options.length] = 
                     new Option(depValueStr, depValueStr);
...
function processRespAction(status, dmPath, mtPath, 
  ...
  var delFlag='false'; //alert(params); //SDN
  ...
  if('error' == status) {
    ...
    if (mtPath == "flowmod_MetaData.xml") {//alert(params);
          alert("Successfully sent the flow modification request to the switch.");
          back();
          return;          
    }
    if('SAVE' == opcode) {
...
function removeSaveDiv() {
   var progressPanelDiv = this.parent.parent.frameTop.tBar.document.getElementById("progressPanelDiv");
   if (progressPanelDiv != null) {
       this.parent.parent.frameTop.tBar.document.body.removeChild(progressPanelDiv);        
   }
   var greyTopPanelDiv = this.parent.parent.frameTop.tBar.document.getElementById("greyTopPanelDiv");
   if (greyTopPanelDiv != null) {
       this.parent.parent.frameTop.tBar.document.body.removeChild(greyTopPanelDiv);        
   }
...
function removeAllGreyDivs() {
  ...
  if (window.name == "newW") {
    ...
    greyTopPanelDiv = opener.parent.parent.frameTop.tBar.document.getElementById("greyTopPanelDiv");
  } else {
  ...
  if (greyTopPanelDiv != null) {
    window.top.frameTop.tBar.document.body.removeChild(greyTopPanelDiv);        
  }
} // removeAllGreyDivs
...
function removeGreyDiv() {//alert(opener);
        ...
        if (window.name == "newW") {
            greyLeftPanelDiv = opener.parent.parent.frameBottom.leftPanel.document.getElementById("greyLeftPanelDiv");
            greyRightPanelDiv = opener.document.getElementById("greyRightPanelDiv");   
            //alert(greyRightPanelDiv);         
            greyTopPanelDiv = opener.parent.parent.frameTop.tBar.document.getElementById("greyTopPanelDiv");
        } else {
            greyRightPanelDiv = opener.document.getElementById("greyRightPanelDiv");            
        }//alert(greyRightPanelDiv.innerHTML);

        if (greyRightPanelDiv != null) {//alert("remove right grey div");
        ...
        if (greyTopPanelDiv != null) {
            opener.parent.parent.frameTop.tBar.document.body.removeChild(greyTopPanelDiv);        
        }
}
  ...
  //load the iframes with tables...
   ...
   for(var l = 0; l < iframes.length; l++) {
     ...
     ucmReqObj = new UCMReq(dmpathResoved, "", mtpathStr,
                         ...
                         getHostID());
     iframes[l].setAttribute('name', dmpathResoved + "$#SDN#$" +
				     "" + "$#SDN#$" +
				     mtpathStr + "$#SDN#$" +
				     renderingXSL + "$#SDN#$" +
				     renderingXSL + "$#SDN#$" +
				     "" + "$#SDN#$" +
				     keyResolvedFinal + "$#SDN#$" +
				     "GET" + "$#SDN#$" +
				     getHostID() );
     
     iframes[l].src = "redirectformtable.htm";  //alert("iframe loading..."); 
     //alert(iframes[l].contentWindow.document.readyState);
  }  
} // updateUIContent 
...
function loadMoreDivsDefaultInnerHTML() {
  var frm = document.forms[1];
  for (i = 0; i < frm.elements.length; i++) {//alert(frm.elements[i].value);
      if (frm.elements[i].type == "hidden" && frm.elements[i].value == "moreDivInnerHTML") {    
          //frm.elements[i].value = document.getElementById(frm.elements[i].name).parentNode.innerHTML;
          //alert(frm.elements[i].value);
          //innerHTMLArray
            innerHTMLArray[frm.elements[i].name] = document.getElementById(frm.elements[i].name).parentNode.innerHTML; //alert(innerHTMLArray[frm.elements[i].name]);
            //document.body.removeChild(document.getElementById(frm.elements[i].name));       
...
function createDivMore(objId, thisDivRef) {//SDN
  //alert(thisDivRef.parentNode);
  var div1 = document.getElementById(objId);
  if (div1) {
             //alert(div1.id);
             divIDsuffix++;
             var moreDivIdStr = div1.id + "_" + divIDsuffix;
             //alert(moreDivIdStr);
             //alert(div1.parentNode.innerHTML); 
             var  htmlString = innerHTMLArray[div1.id]; //alert(htmlString);
            //var htmlString = eval("document.forms[1]." + div1.id + ".value");alert(htmlString);          
            //var htmlString = div1.parentNode.innerHTML;
            htmlString = htmlString.replace("'" + div1.id + "'", "'" + moreDivIdStr + "'");
            htmlString = htmlString.replace('"' + div1.id + '"', '"' + moreDivIdStr + '"');
            //alert(htmlString);
    var tmpHtmlStrings = htmlString.split("toggleContent(");
    //alert(tmpHtmlStrings.length);
    var useParentStr = "", suffixChildStr = ""; 
    if (tmpHtmlStrings.length > 2) {
      for (var xy = 0; xy < tmpHtmlStrings.length; xy++) {
          //alert("tmpHtmlStrings["+xy+"]= " + tmpHtmlStrings[xy]);
          if (tmpHtmlStrings[xy].indexOf("'yes');") != -1) {
             if (useParentStr == "") {
             useParentStr = tmpHtmlStrings[xy].substring(1, tmpHtmlStrings[xy].indexOf("'yes');") - 3);
             //alert("useParentStr: " + useParentStr);
             }
             else {
             suffixChildStr = tmpHtmlStrings[xy].substring(1, tmpHtmlStrings[xy].indexOf("'yes');") - 3);
             //alert("suffixChildStr: " + suffixChildStr);
             //use the strings for replace in the var htmlString
             if (useParentStr != "" && suffixChildStr != "") {//alert("adding suffix...");
             /*if (tmpHtmlStrings[xy].indexOf("moreDivInnerHTML") != -1) {
                alert("Hidden field found, remove..." + tmpHtmlStrings[xy]);
             }*/
             htmlString = htmlString.replace("'" + suffixChildStr + "'", "'" + useParentStr + "_" + suffixChildStr + "'");
             htmlString = htmlString.replace('"' + suffixChildStr + '"', '"' + useParentStr + "_" + suffixChildStr + '"');     
             useParentStr = ""; suffixChildStr = ""; 
             }      
             }
          }
      }
  ...
  moreDiv.innerHTML = htmlString; //alert(htmlString);
  thisDivRef.parentNode.parentNode.appendChild(moreDiv);
  //focus/scroll to new Div...
  //alert("div offset: " + moreDiv.offsetTop);alert("cY: " +cY);
  //alert(window.screen.availTop + cY + moreDiv.offsetTop);
  this.window.scrollTo(2000);
  }
}
// Toggle Form div
function toggleContent(objId, isClicked) {
  ...
  if (div1) {
    ...
    if((grppath != '') && (toggleClicked == "no") && pkeys != null) 
    {
     ...
     if (div1.style.display == 'none') {
      ...
      setAttribute(div1.getElementsByTagName("input"),'visible','true');
      setAttribute(div1.getElementsByTagName("select"),'visible','true');
      //set sub-div elements visible='false'
        var subDivs = div1.getElementsByTagName("div");
  	for (sd = 0; sd < subDivs.length; sd++) {
      		setAttribute(subDivs[sd].getElementsByTagName("input"),'visible','false');
      		setAttribute(subDivs[sd].getElementsByTagName("select"),'visible','false');
        }      
     } else {
  ...
  //set focus on the div in action...
  //alert(cY);
  /*if(self.pageYOffset) {
    //self.pageXOffset = div1.style.left;
    self.pageYOffset = cY + "px";    
  } else if(document.documentElement && document.documentElement.scrollTop) {
    //document.documentElement.scrollLeft;
    document.documentElement.scrollTop = cY + "px";    
  } else if(document.body) {
    //document.body.scrollLeft;
    document.body.scrollTop = cY + "px";    
  }*/
...
function greyForSave() {
  var greyTopPanelDiv = this.parent.parent.frameTop.tBar.document.createElement("div");
  greyTopPanelDiv.className = "greyOutDiv";
  greyTopPanelDiv.id = "greyTopPanelDiv";
  this.parent.parent.frameTop.tBar.document.body.appendChild(greyTopPanelDiv);
  ...
  var pDiv = this.parent.parent.frameTop.tBar.document.createElement("div");
  pDiv.className = "progressDiv";
  pDiv.id = "progressPanelDiv";  
  this.parent.parent.frameTop.tBar.document.body.appendChild(pDiv);
...
function saveAndLogout(flg) {
   ...
   greyForSave();//SDN
    if (defaultHostType != null && defaultHostType == "SingleDevice") {
        setTimeout("submitFormRequest('', 'SAVE', 'on_director');", 250);
    } else {
      var hostname = getHostID();
      if(hostname == "localhost") {
          submitFormRequest('', 'SAVE', 'mgmt');
      } else {
          submitFormRequest('', 'SAVE', 'on_director');
      } 
    }  
}
...
function save() {
  ...
  if(true == cfmSave){     
    greyForSave();//SDN
    if (defaultHostType != null && defaultHostType == "SingleDevice") {//alert("save");
        setTimeout("submitFormRequest('', 'SAVE', 'on_director');", 250);
    } else {
       var hostname = getHostID();
       if (hostname == "localhost") {
...
function factoryReset(){
  submitFormRequest('', 'FACRES', 'on_director');
}
...
function back(){
  ...
  if (this.window.name.indexOf("newW") == -1) {//alert("here1");
  ...
  if (this.window.name.indexOf("newW") != -1) {//alert("here2");
    ...
    window.open('','_parent','');
    window.close();
  } else if(null != parntFrm) {//alert("here3");
    removeAllGreyDivs();
...
function sumbitFrmReq(dmPath, opcode, mtpath, spg, fpg, 
  ...
  if(tgt == "ldiv") {
    ...
    var greyLeftPanelDiv = null;
    var greyTopPanelDiv = null;
    var winWidth = 700, winHeight = 600;
    var relScreenX = 0, relScreenY = 0;
    var newWindowName = "";
    if (window.name == "rightPanel") {//SDN
      newWindowName = "newW";
      ...
      greyTopPanelDiv = parent.parent.frameTop.tBar.document.createElement("div");
      greyTopPanelDiv.className = "greyOutDiv";
      greyTopPanelDiv.id = "greyTopPanelDiv";
      parent.parent.frameTop.tBar.document.body.appendChild(greyTopPanelDiv);
    ...
    } //alert("newWindowName= " + newWindowName);//SDN
      ...
      var winOpen = window.open(url, newWindowName, 
          "width=800, height=600, toolbar=0, resize=1, scrollbars=1, status=0, location=0, menubar=0");
...
function createAddEditPop(doc, name, iFrmOb) {
  ...
  popDiv.id = name;
  //AssignLeftPosition(doc);alert(doc);
...
function setTargetView(cell) {
  ...
  //tgtID = iFrmOb.getAttribute("tblid"));//SDN
  if(parent.opener) {
  ...
  if(tgt == "ldiv") {
    ...
    if(window.top.frameBottom) {//alert("frameBottom");
...
function setTblValueInstance(cell, dmpath, cookie, mtpath, opcode, 
  ...
  if(tgt == "ldiv") {
    ...
    setIfrmID(tgtID +"_ifrm");
    target = tgt;
    var div1 = window.top.frameBottom.rightPanel.document.getElementById(tgtID);//alert(div1.innerHTML);
    toggleLdivs(cell);
    if (div1.style.display == 'none') {
      div1.style.display = "block";
