...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
      <html>
       <head>
        ...
        <![CDATA[        
          ...
          function init(status, msg, cookie, cnfType) {
           ...
           function sendReq(cookie, vrtInstVar, applyOpt, keys) {//alert(applyOpt);             
                    ...
                    var tempPostStr ="";
                         ...
                         && (f.elements[i].getAttribute("visible") =='true'))
                     {
                       ...
                       //alert("parentKeys " + parentKeys);
                        ...
                        //dmpath += "!" + eleOptCode; //SDN
                        //tempPostStr = keyValArray[dmpath];//SDN...added below
                        //alert("keyValArray[dmpath] " + keyValArray[dmpath]);
                        var dmpathExt = "";
			var parDiv = f.elements[i].parentNode.parentNode.parentNode.parentNode.parentNode;//alert(parDiv.id);
                        if (parDiv.id) {
                            if (parDiv.getAttribute("mtpath") && parDiv.getAttribute("class") == "grpDiv") {
                               //alert(parDiv.getAttribute("mtpath"));alert(parDiv.getAttribute("class"));                               
                               dmpathExt = "_$$$_" + parDiv.id;
                        ...
                        if(!tempPostStr) {//alert("new/unique dmpath");
            ...
            { //alert(dmArray[k]);       
               ...
               dmPath = dmPath.substring(0, dmPath.lastIndexOf("_$$$_"));//SDN
          ...
          if(finalPostStr != '') {
            ...
            if (f1.action && f1.action.type != "hidden" && f1.action != "" && f1.action.indexOf("ucm") == -1) {//alert("h1");
	     ...
	     //f.target = "_parent";alert(f.target);
              if(applyOpt != '' && (applyOpt == "DONE"
                                    || applyOpt == "COMMIT"
                                    || applyOpt == "CANCEL"
                                    || applyOpt == "REVOKE")) {
       ...
       function procParentKeys(dmpath,parentKeys) {
           ...
           for(x=0;x<pathTokens.length;x++)
           {
              ...
              if(pKeys[x] != null && pKeys[x] != '')
              {
                ...
                keys = temp.split(',');//alert(keys.length);
                if(keys.length == 1 && (document.getElementsByName(keys[0]).length > 0))
                {//alert("a");
                  var keyValStr = document.getElementsByName(keys[0])[0].value;//alert("keyValStr:"+keyValStr);
                ...
                {//alert("b");alert(gKeyArray.length);
                        ...
                        if(f.useparenttx) {
                         ...
                         if(null != parentTx) {
                           if(parentTx.indexOf(keys[y]) != -1) {
                              //alert("useparenttx keys[y] " + keys[y]);
       ...
       function loadParentKeys() {//alert("loadParentKeys()");
             if (document.forms[0].dmpath.value != "") {
                 var gDmPath = document.forms[0].dmpath.value;//alert(gDmPath);
      ...
      <body onunload="removeGreyDiv();"><!-- Onload function calls below --> 	
       ...
       <xsl:attribute name="onload">
         <xsl:text>renameBackButton();setProductNameAsTitle(this.document);greyDisabled();loadParentKeys();</xsl:text>
         <xsl:text>dynamiclist('</xsl:text>
           <xsl:value-of select="$cookieName" />
         <xsl:text>');updateUIContent('</xsl:text>
           <xsl:value-of select="$cookieName" />
         <xsl:text>');loadMoreDivsDefaultInnerHTML();</xsl:text>
       </xsl:attribute>
