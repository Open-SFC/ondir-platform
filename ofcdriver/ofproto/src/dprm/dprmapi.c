...
extern int32_t get_number_of_ttp_tables(char *ttp_name_p, uint32_t *no_of_tbls_p);
                                                  ...
                                                  uint64_t* output_handle_p)
{
  ...
  *output_handle_p = ((*output_handle_p <<32) | (index));
  ...
  /** add domain to ttprm module **/
  add_domain_to_ttprm(domain_info_p->ttp_name,domain_info_p->name, domain_entry_p->domain_handle);
...
int32_t dprm_get_ttp_name_by_dpid(uint64_t dpid, char *ttp_name_p)
{
  uint32_t hashkey=0;
  uint16_t offset=0;
  int32_t status=DPRM_FAILURE;
  ...
  MCHASH_BUCKET_SCAN(dprm_datapath_table_g,hashkey,datapath_entry_p,struct dprm_datapath_entry *,offset)
  {
	  ...
	  if ( datapath_entry_p->domain == NULL)
	  { 	
		  CNTLR_RCU_READ_LOCK_RELEASE();  
		  return status;
	  }
	  if ( datapath_entry_p->domain->ttp_name[0] == '\0')
	  { 	
		  CNTLR_RCU_READ_LOCK_RELEASE();  
		  return status;
	  }
	  strncpy( ttp_name_p, datapath_entry_p->domain->ttp_name ,CNTLR_MAX_TTP_NAME_LEN);
	  CNTLR_RCU_READ_LOCK_RELEASE();
	  return DPRM_SUCCESS;
  }
  CNTLR_RCU_READ_LOCK_RELEASE();
...
/*******************************************************************************************************/
int32_t
dprm_set_async_cfg_ctl_db(uint64_t dp_handle, struct ofp_async_config *async_msg_p)
{
  struct   dprm_datapath_entry *datapath_info = NULL;
  dprm_debug("\nGetting async config info from DPRM\n");
  CNTLR_RCU_READ_LOCK_TAKE();
  if(get_datapath_byhandle(dp_handle,&datapath_info) == DPRM_ERROR_INVALID_DATAPATH_HANDLE)
  {
    CNTLR_RCU_READ_LOCK_RELEASE();
    return OF_FAILURE;
  }
  memcpy(&datapath_info->async_msg_db_info, async_msg_p, sizeof(struct ofp_async_config));
  CNTLR_RCU_READ_LOCK_RELEASE();
  return OF_SUCCESS;
}

int32_t 
dprm_get_async_cfg(uint64_t dp_handle, struct ofp_async_config *async_msg_p)
{
  struct   dprm_datapath_entry *datapath_info = NULL;
  dprm_debug("\nGetting async config info from DPRM\n");
  CNTLR_RCU_READ_LOCK_TAKE();
  ...
  memcpy(async_msg_p, &datapath_info->async_msg_db_info, sizeof(struct ofp_async_config));
  CNTLR_RCU_READ_LOCK_RELEASE();
  return OF_SUCCESS;
}
                                  ...
                                  uint64_t *port_handle_p)
{
  ...
  *port_handle_p = ((*port_handle_p <<32) | (index));
  ...
  memcpy(&port_entry_p->port_info, &dprm_port_p->port_desc,sizeof(struct ofl_port_desc_info));
...
int32_t dprm_delete_port_from_datapath(uint64_t datapath_handle,uint64_t port_handle)
{
  ...
  printf("\r\n dprm_delete_port_from_datapath: dprm_delete_port_from_datapath");
  ...
  dprm_send_port_detach_events(datapath_info_p, port_info_p);
...
/****************************************************************************************************/
int32_t dprm_get_port_info_by_port_id(uint64_t datapath_handle, uint16_t port_id, struct ofp_port_mod *port_info)
{
    struct dprm_port_entry *dp_port_entry = NULL;
  uint64_t    port_handle = 0;
  int32_t return_value = DPRM_FAILURE; 

  return_value  = dprm_get_port_handle_by_port_id(datapath_handle , port_id, &port_handle);
  if(DPRM_SUCCESS != return_value)
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR, "Failed to get port handle for port id = %u \r\n", port_id);
    return DPRM_FAILURE;
  }

  CNTLR_RCU_READ_LOCK_TAKE();
  return_value = get_datapath_port_byhandle(datapath_handle, port_handle, &dp_port_entry);
  if(DPRM_SUCCESS != return_value)
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"Failed to get port info for port id = %u \r\n", port_id);
    CNTLR_RCU_READ_LOCK_RELEASE();
    return DPRM_FAILURE;
  }
  CNTLR_RCU_READ_LOCK_TAKE();
  ...
  memcpy(port_info->hw_addr, dp_port_entry->port_info.hw_addr, OFP_ETH_ALEN);
