...
int32_t dprm_deregister_namespace_notifications(uint32_t notification_type,
                                             dprm_namespace_notifier_fn namespace_notifier_fn)
{
	...
	do
	{
		...
		/* Add Application to the list of switch Applications maintained per notification type. */
		offset=DPRM_NOTIFICATIONS_APP_HOOK_LISTNODE_OFFSET;
		OF_LIST_SCAN(namespace_notifications[notification_type],app_entry_p,struct dprm_notification_app_hook_info*,DPRM_NOTIFICATIONS_APP_HOOK_LISTNODE_OFFSET)
		{
			if(app_entry_p->call_back != namespace_notifier_fn)
			{
				prev_app_entry_p = app_entry_p;
				continue;
			}

			OF_REMOVE_NODE_FROM_LIST( namespace_notifications[notification_type],app_entry_p,prev_app_entry_p,offset );
			mempool_release_mem_block(dprm_notifications_mempool_g,(uchar8_t*)app_entry_p,app_entry_p->heap_b);
			status=OF_SUCCESS;
			break;
		}
...
int32_t dprm_send_port_detach_events(struct  dprm_datapath_entry* datapath_info_p,
                struct    dprm_port_entry* dprm_port_p)
{
	...
	do
	{
		...
		/* Send namespace port delete notification to the registered Applications. */
		OF_LIST_SCAN(namespace_notifications[DPRM_NAMESPACE_PORT_DETACHED],app_entry_p,struct dprm_notification_app_hook_info*, lstoffset)
		{
		...
		{
			((dprm_namespace_notifier_fn)(app_entry_p->call_back))(DPRM_NAMESPACE_PORT_DETACHED,namespace_notification_data,app_entry_p->cbk_arg1,app_entry_p->cbk_arg2);
		}
	}while(0);
                ...
                struct    dprm_port_entry* dprm_port_p)
{
	...
	do
	{
		...
		namespace_notification_data.dp_id= datapath_info_p->dpid;
		namespace_notification_data.port_id= dprm_port_p->port_id;
                memcpy(&namespace_notification_data.hw_addr, dprm_port_p->port_info.hw_addr,OFP_ETH_ALEN);
		/* Send namespace add notification to the registered Applications. */
