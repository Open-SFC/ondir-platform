...
int32_t pkt_mbuf_alloc_stats_g=0;
int32_t pkt_mbuf_free_stats_g=0;
#endif

struct pktbuf_callbacks *pktbuf_cbks = NULL;
/* INCLUDE_END **************************************************************/
...
#define CNTRL_RESOURCE_CONF_FILE   "/usr/local/ondir_platform/2.0/conf/cntrl-rconfig.xml"
...
struct peth_char_dev_info *peth_char_dev_inst_g = NULL;
...
/*GLOBAL_END    ************************************************************/
struct library_handles library_handle_g[MAX_LIB_PATHS];
...
of_controller_init(void)
{
  ...
  do
  {   
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"timer_module_init..............");
    ...
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"timer_module_init complete");
    of_controller_setup_signal_handlers();

    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"of_view_init.............");
    of_view_init();
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"of_view_init completed");
#ifdef CNTLR_CRM_SUPPORT
#if 0
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"crm_init..............");
    retval = crm_init();
    if(retval != OF_SUCCESS)
    {
    ...
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"ttprm_init..............");
    retval = ttprm_init();
    if(retval != OF_SUCCESS)
    {
      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"ttprm_init()  - Failure");
      status = OF_FAILURE;
      break;
    }

    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"cntlr_genid_init..............");
    retval = cntlr_genid_init();
    if(retval != OF_SUCCESS)
    {
      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"cntlr_genid_init()  - Failure");
      status = OF_FAILURE;
      break;
    }
...
int main(int argc, char *argv[])
{
  ...
  memset(&library_handle_g,0,sizeof(struct library_handles)*MAX_LIB_PATHS);
  ...
  if(cntlr_epoll_init(NO_FD_SUPPORTED) == OF_FAILURE)
  {
  ...
  memset(dir_path, 0, CNTLR_MAX_DIR_PATH_LEN);
  strcpy(dir_path, CNTLR_DIR);
  strcat(dir_path, CNTLR_SHARED_SUBTTP_DIR);
  cntlr_load_shared_libraries(dir_path);

  /* apidispatcher shared lib */
  memset(dir_path, 0, CNTLR_MAX_DIR_PATH_LEN);
  strcpy(dir_path, CNTLR_DIR);
  strcat(dir_path, CNTLR_SHARED_APID_DIR);
  cntlr_load_shared_libraries(dir_path);

  /* main ttp lib */
  memset(dir_path, 0, CNTLR_MAX_DIR_PATH_LEN);
  strcpy(dir_path, CNTLR_DIR);
  strcat(dir_path, CNTLR_SHARED_MAINTTP_DIR);
  cntlr_load_shared_libraries(dir_path);



  /* linux integration shared lib */
  memset(dir_path, 0, CNTLR_MAX_DIR_PATH_LEN);
  strcpy(dir_path, CNTLR_DIR);
  strcat(dir_path, CNTLR_SHARED_LIM_DIR);
  cntlr_load_shared_libraries(dir_path);
...
int32_t  cntlr_load_shared_libraries(char *dir_path_p)
{
	...
	void    *lib_handle_p;
        int32_t libpath_index,status=OF_SUCCESS;
	...
	do
	{
		...
		for (libpath_index=0; libpath_index < MAX_LIB_PATHS;  libpath_index++)
		{
			if(library_handle_g[libpath_index].valid_b == FALSE)
			{
				library_handle_g[libpath_index].max_lib_index=0;
				break;
			}

			if(!strcmp(library_handle_g[libpath_index].dir_path, dir_path_p))
			{
				OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "Shared Lib DIR Path = %s already loaded",dir_path_p);
				status=OF_FAILURE;
				break;
			}
		}
		...
		if ( libpath_index == MAX_LIB_PATHS)
		{
			OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR, "no space for Shared Lib DIR Path = %s",dir_path_p);
			status=OF_FAILURE;
			break;
		}

		library_handle_g[libpath_index].valid_b=TRUE;
		strcpy(library_handle_g[libpath_index].dir_path, dir_path_p);
		...
		for( ii = 0; ii < no_of_files; ii++)
		{
			...
			OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"Loading library:%s",path);

			/* Open the file */
			lib_handle_p = dlopen(path,RTLD_NOW|RTLD_GLOBAL);
			...
			if(!lib_handle_p)
			{
				...
				OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "dlopen failed for path:%s",path);
				continue;
			}

			/* Get the Registration Handler */
			ThreadInitHook = ( CNTLRREGISTERHOOK)dlsym(lib_handle_p,"cntlr_shared_lib_init");
			if ((error = dlerror()) != NULL)
			{
				fprintf (stderr, "%s:dlsym() returned error: %s\n",__FUNCTION__,error);
				OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "dlsym failed for path:%s",path);
				continue;
			}
			...
			if(ThreadInitHook)
				ThreadInitHook();
			library_handle_g[libpath_index].handle_p[library_handle_g[libpath_index].max_lib_index]=lib_handle_p;
			library_handle_g[libpath_index].max_lib_index++;
		}
...
int32_t  cntlr_unload_shared_libraries(char *dir_path_p)
{
	char    *error;
	void    *lib_handle_p;
        int32_t libpath_index, index;

	CNTLRREGISTERHOOK ThreadDeInitHook; 
	...
	for (libpath_index=0; libpath_index < MAX_LIB_PATHS;  libpath_index++)
	{
		...
		OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "libpath %s max libs %d", library_handle_g[libpath_index].dir_path,
						library_handle_g[libpath_index].max_lib_index);
		for (index=library_handle_g[libpath_index].max_lib_index -1 ; index >= 0;  index--)
		{
			/* Open the file */
			lib_handle_p = library_handle_g[libpath_index].handle_p[index];

			if(!lib_handle_p)
			{
				OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "lib handle is null for index %d", index);
				continue;
			}

			/* Get the Registration Handler */
			ThreadDeInitHook = ( CNTLRREGISTERHOOK)dlsym(lib_handle_p,"cntlr_shared_lib_deinit");
			if ((error = dlerror()) != NULL)
			{
				OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "dlsym failed for path:%s",library_handle_g[libpath_index].dir_path);
			        dlclose(lib_handle_p);
				continue;
			}
...
{  
  ...
  new_signal_action.sa_handler = (sahandler)of_controller_handle_signal;
...
void of_controller_handle_signal (int16_t signal_num_i)
{
  ...
  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"* * * * * *STOPPING CONTROLLER* * * * * * *");

  of_set_trace_level(OF_LOG_MODULE_ALL,OF_LOG_ERROR);
  ...
  strcpy(dir_path, CNTLR_DIR);
  strcat(dir_path, CNTLR_SHARED_APP_DIR);
  cntlr_unload_shared_libraries(dir_path);
#endif
...
//  cntlr_transprt_deinit();
 ...
 /* add all clean up/De-Init/De-Registration functions here */
   ...
   switch(signal_num_i)
  {
    ...
    case SIGHUP:
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"Recieved the Hangup Signal");
      }
      break;
    case SIGSEGV:
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"Recieved the SIGSEGV Signal");
      }
      break;
    case SIGINT:
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"Recieved the Interuppt Signal");
        /* To generate reports like memory checks some tools like valgrind expect normal termination
         *  of process, to facilitate this we are calling exit(0) */
      }
      break;
    case SIGTERM:
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"Recieved the TERM Kill Signal");
      }
      break;
    case SIGTSTP:
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"Recieved the keyboard control Z Signal");
      }
      break;
    case SIGKILL:
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"Recieved the kill Signal");
      }
      break;
    case SIGILL:
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"Recieved the SIGILL Signal");
      }
      break;
    case SIGABRT:
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_CRITICAL,"Recieved the SIGABRT Signal");
      }
      break;
  }
