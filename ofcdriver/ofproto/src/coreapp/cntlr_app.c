...
//manish of_app_register_async_event_callback_fun of_app_async_event_callback_fun_g = NULL;
...
/*manish
   if(of_app_async_event_callback_fun_g)
    of_app_async_event_callback_fun_g(domain_handle);
*/
   return OF_SUCCESS;
}
...
void of_async_config_reply_cbk_fn(uint64_t  controller_handle,
      ...
      uint64_t  datapath_handle,
      struct of_msg *msg,
      struct ofp_async_config *async_msg,
      void     *cbk_arg1,
      void     *cbk_arg2,
      uint8_t   status)
{
   ...
   if( status == OF_RESPONSE_STATUS_SUCCESS)
   {
      memcpy(&datapath->async_msg_db_info, async_msg, sizeof(struct ofp_async_config));
      cntrl_ucm_send_multipart_response(OFPT_GET_ASYNC_REPLY, async_msg, TRUE);
   }
      ...
      uint8_t auxilaryID)
{
   ...
   return ofp_prepare_and_send_echo_request (datapath_handle,NULL,cntlr_echo_reply_clbk_fn,&auxilaryID,NULL);
}
...
/* manish
void of_app_register_async_event_function(of_app_register_async_event_callback_fun callback_fun)
{
  of_app_async_event_callback_fun_g = callback_fun;
  return;
}
*/
