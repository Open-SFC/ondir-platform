 ...
 * * * * * * * * * * * * * * Global Variables * * * * * * * * * * * * * * * * * 
 *******************************************************************************/
void     *nem_peth_2_dpid_db_mempool_g = NULL;
uint32_t nem_peth_2_dpid_buckets_g;
struct   mchash_table* nem_peth_2_dpid_db_table_g = NULL;

/******************************************************************************
 * * * * * * * * * * * * * * Function Prototypes * * * * * * * * * * * * * * * * * 
 *******************************************************************************/
void nem_free_peth_2_dpid_dbentry_rcu(struct rcu_head *map_entry_rcu_p);


extern uint32_t nem_get_hashval_by_nsid_and_peth_id(uint32_t nsid, uint32_t peth_id,uint32_t no_of_buckets);
...
int32_t nem_peth_2_dpid_db_init()
{
   ...
   do
   {
      ...
      ret_value=nem_peth_2_dpid_db_mempool_init();
      ...
      ret_value=nem_peth_2_dpid_db_mchash_table_init();
      if(ret_value != NEM_SUCCESS)
      {
...
int32_t nem_peth_2_dpid_db_deinit()
{
   int32_t status=NEM_SUCCESS;
   int32_t ret_value;
   ...
   do
   {
      ret_value=nem_peth_to_dpid_unmap_all_entries();
      ...
      ret_value=  mchash_table_delete(nem_peth_2_dpid_db_table_g);
      if( CNTLR_UNLIKELY(ret_value != OF_SUCCESS) )
      {
      ...
      ret_value=mempool_delete_pool(nem_peth_2_dpid_db_mempool_g);
      if( CNTLR_UNLIKELY(ret_value != OF_SUCCESS) )
      {
...
int32_t nem_peth_2_dpid_db_mempool_init()
{
   ...
   ret_value = mempool_create_pool("nem_peth_2_dpid_db_pool", &mempool_info,
		   &nem_peth_2_dpid_db_mempool_g
		   );
...
int32_t nem_peth_2_dpid_db_mchash_table_init()
{
   ...
   nem_peth_2_dpid_buckets_g = ((db_entries_max / 5)+1);
   ret_value = mchash_table_create( nem_peth_2_dpid_buckets_g,
         db_entries_max,
         nem_free_peth_2_dpid_dbentry_rcu,
         &nem_peth_2_dpid_db_table_g);
...
void nem_free_peth_2_dpid_dbentry_rcu(struct rcu_head *map_entry_rcu_p)
{
   ...
   if(map_entry_rcu_p)
   {
      ...
      mempool_release_mem_block(nem_peth_2_dpid_db_mempool_g,(uchar8_t*)map_entry_p,map_entry_p->heap_b);
      return;
   }
...
int32_t nem_map_peth_to_dpid(struct of_nem_dp_peth_info *dp_peth_info_p,uint64_t*  output_handle_p)
{
   ...
   do
   {
      ...
      if(nem_peth_2_dpid_db_table_g == NULL)
      {
      ...
      if(dp_peth_info_p->peth_name == NULL)
      {
      ...
      if ((0 > dp_peth_info_p->ns_id) || (dp_peth_info_p->ns_id > OF_NEM_MAX_NAMESPACE_ENTRIES))
      {
         OF_LOG_MSG(OF_NEM_LOG, OF_LOG_ERROR, "nsid is invalid");
         status=NEM_ERROR_NAME_LENGTH_NOT_ENOUGH;
         break;
      }


      hashkey = nem_get_hashval_by_nsid_and_peth_id(dp_peth_info_p->ns_id, dp_peth_info_p->peth_id,nem_peth_2_dpid_buckets_g);  

      ret_value=of_nem_peth_2_dpid_db_get_map_entry_handle(dp_peth_info_p->ns_id, dp_peth_info_p->peth_id, output_handle_p);
      if ( ret_value == NEM_SUCCESS)
      {
      ...
      ret_value = mempool_get_mem_block(nem_peth_2_dpid_db_mempool_g,(uchar8_t**)&map_entry_p,&heap_b);
      if(ret_value != MEMPOOL_SUCCESS)
      {
...
//      strcpy(map_entry_p->dp_peth_info.port_name, dp_peth_info_p->port_name);
      ...
      MCHASH_APPEND_NODE(nem_peth_2_dpid_db_table_g,hashkey,map_entry_p,index,magic,hashobj_p,status_b);
      if(FALSE == status_b)
      {
...
int32_t nem_unmap_peth_to_dpid(uint64_t handle)
{
   ...
   do
   {
      ...
      MCHASH_ACCESS_NODE(nem_peth_2_dpid_db_table_g,index,magic,map_entry_p,status_b);
      if(TRUE == status_b)
      {
         ...
         status_b = FALSE;
         offset = DPID_PETH_MAPNODE_HASH_TBLLNK_OFFSET;
         MCHASH_DELETE_NODE_BY_REF(nem_peth_2_dpid_db_table_g,index,magic,struct nem_dpid_and_peth_map_entry *,offset,status_b);
//         CNTLR_RCU_READ_LOCK_RELEASE();
         if(status_b == TRUE)
         {
            status= NEM_SUCCESS;
...
int32_t nem_peth_to_dpid_unmap_all_entries()
{
   ...
   CNTLR_RCU_READ_LOCK_TAKE();
   MCHASH_TABLE_SCAN(nem_peth_2_dpid_db_table_g,hashkey)
   {
      offset = DPID_PETH_MAPNODE_HASH_TBLLNK_OFFSET;
      MCHASH_BUCKET_SCAN(nem_peth_2_dpid_db_table_g,hashkey,map_entry_p,struct nem_dpid_and_peth_map_entry *,offset)
      {
         MCHASH_DELETE_NODE_BY_REF(nem_peth_2_dpid_db_table_g,map_entry_p->index,map_entry_p->magic,struct nem_dpid_and_peth_map_entry *,offset,status_b);
         if(status_b == TRUE)
         {
            status= NEM_SUCCESS;
            OF_LOG_MSG(OF_NEM_LOG, OF_LOG_DEBUG, "deleted success from hash table");
...
int32_t of_nem_peth_2_dpid_db_get_map_entry_handle(uint32_t ns_id, uint32_t peth_id, uint64_t* map_entry_handle_p)
{
   uint32_t hashkey;
   uint16_t offset;
   ...
   do
   {
      ...
      if(!peth_id)
      {
         OF_LOG_MSG(OF_NEM_LOG, OF_LOG_DEBUG, "peth id is null");
         status=NEM_FAILURE;
         break;
      }
      hashkey = nem_get_hashval_by_nsid_and_peth_id(ns_id, peth_id,nem_peth_2_dpid_buckets_g);

      CNTLR_RCU_READ_LOCK_TAKE();
      offset = DPID_PETH_MAPNODE_HASH_TBLLNK_OFFSET;
      MCHASH_BUCKET_SCAN(nem_peth_2_dpid_db_table_g,hashkey,map_entry_p,struct nem_dpid_and_peth_map_entry *,offset)
      {
         if(!((map_entry_p->dp_peth_info.ns_id == ns_id) && (map_entry_p->dp_peth_info.peth_id == peth_id)))
            continue;
//         CNTLR_RCU_READ_LOCK_RELEASE();
...
int32_t of_nem_peth_2_dpid_db_get_map_entry_handle_by_pethname(uint32_t ns_id, char *peth_name, uint64_t* map_entry_handle_p)
{
   uint32_t hashkey;
   uint16_t offset;
   int32_t status= NEM_ERROR_INVALID_NAME;
   ...
   { 
      ...
      if(peth_name == NULL)
      {
         OF_LOG_MSG(OF_NEM_LOG, OF_LOG_DEBUG, "peth name is  null"); 
         status=NEM_FAILURE;
         break;
      }
      hashkey = nem_get_hashval_by_nsid_and_peth(ns_id, peth_name,nem_peth_2_dpid_buckets_g);

      CNTLR_RCU_READ_LOCK_TAKE();
      offset = DPID_PETH_MAPNODE_HASH_TBLLNK_OFFSET;
      MCHASH_BUCKET_SCAN(nem_peth_2_dpid_db_table_g,hashkey,map_entry_p,struct nem_dpid_and_peth_map_entry *,offset)
      {
         if(!((map_entry_p->dp_peth_info.ns_id == ns_id) && (!strcmp(map_entry_p->dp_peth_info.peth_name,peth_name))))
            continue;
//         CNTLR_RCU_READ_LOCK_RELEASE();
...
int32_t of_nem_get_dpid_and_port_id_from_nsid_and_peth_id(uint32_t ns_id, uint32_t peth_id, uint64_t *dp_id_p, uint32_t *port_id_p)
{
   ...
   do
   {
      ...
      if(peth_id == 0)
      {
         OF_LOG_MSG(OF_NEM_LOG, OF_LOG_DEBUG, "input is  NULL");
         status=NEM_FAILURE;
         break;
      }

      if ((0 > ns_id) || (ns_id > OF_NEM_MAX_NAMESPACE_ENTRIES))
      {
      ...
      if(port_name == NULL)
      {
         OF_LOG_MSG(OF_NEM_LOG, OF_LOG_DEBUG, "output is  NULL"); 
         status=NEM_FAILURE;
         break;
      }
#endif
...
//      hashkey = nem_get_hashval_by_nsid_and_peth(ns_id, peth_name,nem_peth_2_dpid_buckets_g);
      ...
      ret_value=of_nem_peth_2_dpid_db_get_map_entry_handle(ns_id, peth_id, &map_entry_handle);
      if ( ret_value != NEM_SUCCESS)
      {
      ...
      ret_value=  of_nem_peth_2_dpid_db_get_exact_map_entry(map_entry_handle,
            &dp_peth_info);
      if ( ret_value != NEM_SUCCESS)
      {
      ...
      *dp_id_p=dp_peth_info.dp_id;
      *port_id_p=dp_peth_info.port_id;
//      strcpy(peth_name, dp_peth_info.peth_name);
...
int32_t of_nem_get_dpid_and_port_id_from_nsid_and_peth_name(uint32_t ns_id, char *peth_name, uint64_t *dp_id_p, uint32_t *port_id_p)
{
   ...
   do
   {
      ...
      ret_value=of_nem_peth_2_dpid_db_get_map_entry_handle_by_pethname(ns_id, peth_name, &map_entry_handle);
      if ( ret_value != NEM_SUCCESS)
      {
         OF_LOG_MSG(OF_NEM_LOG, OF_LOG_DEBUG, "resource doesnt exists"); 
         status = NEM_ERROR_INVALID_HANDLE;
