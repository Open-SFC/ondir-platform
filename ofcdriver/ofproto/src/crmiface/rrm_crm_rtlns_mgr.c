...

{
  ...
  else if((port_type == VMNS_PORT) || (port_type == VMNS_IN_PORT) || (port_type == VMNS_OUT_PORT))
  {
                                            ...
                                            uint64_t*  port2_saferef_p)
{
  ...
  OF_LIST_SCAN((crm_vm_node_p->list_of_logical_switch_side_ports_connected),
                ...
                CRM_VM_PORT_NODE_ENTRY_LISTNODE_OFFSET)
  {
    /* At present we assume only one network per Service VM.
       if(strcmp(crm_vm_port_node_scan_entry_p->vn_name, notification_data.launch.vn_name_p))
          continue;
     */
      ...
      if((ret_val == CRM_SUCCESS) && ((port_info_p->crm_port_type == VMNS_PORT)    ||
                                      (port_info_p->crm_port_type == VMNS_IN_PORT) ||
                                      (port_info_p->crm_port_type == VMNS_OUT_PORT)))
      {
        ...
        ++no_of_ports;
        if(no_of_ports == 1)
          *port1_saferef_p = crm_vm_port_node_scan_entry_p->port_saferef;
        else if(no_of_ports == 2)
        {
          *port2_saferef_p = crm_vm_port_node_scan_entry_p->port_saferef;
           ret_val = CRM_SUCCESS;
           break;
        }
      }
