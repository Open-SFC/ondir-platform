		...
		uint8_t              *peer_closed_conn )
{
	int32_t                     ret_val = OF_SUCCESS;
	int32_t                     status = OF_SUCCESS;
        /* don't change the int32 for curr_len_sent and uint16_t for length*/
        int32_t     curr_len_sent = length;
        int32_t     total_len_sent = 0;
	OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG,"entered");

	do
	{
		ret_val = send(fd,
				(msg + total_len_sent), curr_len_sent, flags);
		if(ret_val == -1)
		{                       
		...
		{
                        curr_len_sent = curr_len_sent - ret_val;
                        total_len_sent += ret_val;
		}
                
	}while(curr_len_sent > 0);

        *length_sent = total_len_sent;
        OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG,"length sent is %d bytes ",*length_sent);
	return status;
}
