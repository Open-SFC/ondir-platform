...
int32_t tls_transport_open ( int32_t fd, of_conn_properties_t *conn_details_p)
{
   ...
   if( (conn_sock_fd = accept(fd, (struct sockaddr *)&rmt_addr, (socklen_t *)&rmt_addr_len)) < 0 )
   {
   ...
   OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR,"TCP CONN ESTABLISHED");
   do
   {
      ...
      pssl_info->ctx = ctx;
      pssl_info->ssl = NULL;
      ret_val = of_ssl_get_sslobject_for_client(conn_sock_fd,
