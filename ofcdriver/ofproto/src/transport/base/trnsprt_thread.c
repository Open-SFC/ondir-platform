      ...
      uint32_t           received_events)
{
   ...
   else if(CNTLR_LIKELY(EPOLL_EVENT_RCVD == msg_event.command))
   {
     OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG,"EPOLL_EVENT_RCVD recevied to set POLLOUT event in epoll \r\n");
     of_register_write_event_to_epoll_fd(channel);
     channel->msg_q_box->bSendEvent = TRUE;
     OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG," channel->msg_q_box->bSendEvent is set to TRUE \r\n");
