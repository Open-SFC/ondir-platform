 ...
 * * * * * * * * * * * * Function Prototypes * * * * * * * * * * * * * * * * * *
 *******************************************************************************/
extern inline int32_t CNTLR_PKTPROC_FUNC of_process_dp_message(cntlr_channel_info_t *channel,
    struct ofp_header    *p_msg_hdr);
extern int32_t of_drain_out_socket_buf(cntlr_channel_info_t  *channel, uint16_t length);
void of_read_and_process_msg_from_dp(cntlr_channel_info_t *channel);
      ...
      uint32_t received_events)
{
   ...
   do
   {
      ...
      if(received_events & ePOLL_POLLIN)
         of_read_and_process_msg_from_dp(channel); /* NSM commented else learnt from Manish */ 
        ...
        (channel->msg_hdr.version == FSLX_VERSION))
      {
	      ...
	      if((channel->msg_hdr.length)&& ((channel->msg_hdr.length - sizeof(struct ofp_header)) > 0))
	      {
		      if(of_drain_out_socket_buf(channel, channel->msg_hdr.length - sizeof(struct ofp_header)) != OF_SUCCESS)
		      {
                  ...
                  printf("\n of_read_and_process_msg_from_dp -- of_drain_out_socket_buf \r\n");
              }
	      }
