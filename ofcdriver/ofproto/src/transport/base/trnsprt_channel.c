      ...
      )
{
   ...
   do
   {
      ...
      CNTLR_LOCK_INIT(channel_p->fd_lock);
...
dprm_free_datapath_channels(struct dprm_datapath_entry  *datapath_p)
{
   ...
   dprm_dp_channel_entry_t        *dp_channel = NULL;
   dprm_dp_channel_entry_t        *aux_channel = NULL;
   ...
   do
   {
      ...
      {
         dp_channel = datapath_p->aux_channels;
         while(datapath_p->curr_no_of_aux_chanels > 0)
         {
            CNTLR_RCU_DE_REFERENCE(dp_channel,aux_channel);
            MCHASH_ACCESS_NODE(aux_channel->conn_table,aux_channel->conn_safe_ref.index, aux_channel->conn_safe_ref.magic,channel,is_ch_access_status);
            if( CNTLR_UNLIKELY(is_ch_access_status == FALSE) )
            {
               OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_WARN,"channel fd %d saferef index %d magic %d", channel->trnsprt_fd, aux_channel->conn_safe_ref.index, aux_channel->conn_safe_ref.magic);                                     continue; 
            }         
            fsm_send_channel_event(channel, OF_XPRT_PROTO_CHANNEL_DP_DETACH_EVENT); 
            datapath_p->curr_no_of_aux_chanels--; 
            dp_channel = dp_channel->next;        
         }
