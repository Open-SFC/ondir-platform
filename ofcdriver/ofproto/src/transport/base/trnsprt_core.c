...
int32_t CNTLR_PKTPROC_DATA cntlr_send_epoll_write_event_2_xprt_conn(cntlr_channel_info_t *channel)
{
   of_msg_q_box_t             *msg_q_box;
   int32_t                     retval = OF_SUCCESS;
   int32_t                     status = OF_SUCCESS;
   ...
   do
   {
      if(likely(channel->epoll_wr_enabled))
      {
         OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_WARN," Already channel fd is registered with write event POLLOUT \r\n ");
         return OF_SUCCESS;
      }
      ...
      if(msg_q_box->bSendEvent)
      {
         ...
         msg_event.command              = EPOLL_EVENT_RCVD;
         msg_event.msg.sender_thread_id = cntlr_thread_index;
         ...
         msg_q_box->bSendEvent = FALSE;
         OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR,"msg_q_box->bSendEvent set to FALSE successfully !!!");
         close(socket);
      }
   }
   while(0);
      ...
      cntlr_conn_node_saferef_t  *conn_safe_ref)
{
   ...
   CNTLR_RCU_READ_LOCK_TAKE();
   OF_LOG_MSG(OF_LOG_XPRT_PROTO,OF_LOG_DEBUG,"saferef is %d , magic num is %d",conn_safe_ref->index,conn_safe_ref->magic);
   do
   {
      ...
      uint16_t              len_sent_currently)
{
   ...
   OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG,"add to epoll for write event");
   CNTLR_RCU_READ_LOCK_TAKE();
   do
   {
      ...
      if(is_new_msg == TRUE)
      {
         ...
         OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG,"Appended to list");
      }

      if(!channel->epoll_wr_enabled)
      {

         if((unlikely(self_thread_info->thread_id == channel->thread->thread_id)))
         {
           /* Only transport thread, whose epoll context has  owned this socket fd,  can directly chnage it's attributes */
           ...
           OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG," epoll_wr_enabled  is set to TRUE  \r\n");
           channel->epoll_wr_enabled = TRUE;
         }
         else
         {
           /* send the epoll write event to the transport thread, whose epoll context has contained this socket fd  */
           if(OF_FAILURE == cntlr_send_epoll_write_event_2_xprt_conn(channel))
           {
             OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR," sending epoll event to transport thread failed  \r\n");
             status = OF_FAILURE;
             break;
           }
         }
   ...
   if(status == OF_FAILURE)
   {
      OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR," of_add_2_epoll_4_wr_event failed   \r\n");
#if 0
      if(msg)
   ...
   void
of_register_write_event_to_epoll_fd(cntlr_channel_info_t *channel)
{
   //cntlr_send_msg_buf_t *send_msg_buf = NULL;
   epoll_user_data_t    user_data;
   uint32_t             intrested_events;
   ...
   do
   {
      ...
      {
         OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG," epoll_wr_enabled is already set to TRUE. POLLOUT Already registered  \r\n");
      }
   }
   while(0);
   CNTLR_RCU_READ_LOCK_RELEASE();
...
of_send_all_msgs_waiting_on_chn_to_dp(cntlr_channel_info_t *channel) 
{
   ...
   if(OF_LIST_COUNT(channel->send_msg_list) == 0)
   {
      ...
      OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR," epoll_wr_enabled is set to FLASE \r\n");
