...
extern int32_t timer_create_and_start(struct saferef* container_saferef_p,struct tmr_saferef* tmr_saferef_p, uint8_t periodic_b,tmr_time_t time_out_p, tmr_cbk_func func_p, void *arg1_p,void *arg2_p);
extern int32_t  timer_stop_tmr(struct saferef* container_saferef_p, struct tmr_saferef* tmr_node_saferef_p);

int32_t of_process_feature_reply_msg(cntlr_channel_info_t        *channel,
      ...
      struct ofp_switch_features  *sw_feature_msg)
{
   ...
   do
   {
      ...
      OF_LOG_MSG(OF_LOG_XPRT_PROTO,OF_LOG_ERROR,"AUx id rcvd is :%d",sw_feature_msg->auxiliary_id);
      OF_LOG_MSG(OF_LOG_XPRT_PROTO,OF_LOG_ERROR,"AUx id rcvd is :%d",channel->auxiliary_id);
      ...
      { 
        ...
        retval = cntlr_get_async_msg_cfg(channel->dp_handle);
        if(retval != OF_SUCCESS)
        {
          OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR,
               "\nGet Async configuration failed!\n");
          status = OF_FAILURE;
          break;
        }
       }
   ...
   if(status == OF_FAILURE)
   {
      ...
      OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR," CLOSING THE CHANNEL  \r\n");
...
// After Handshake only Echo request should allow 
      ...
      void *cbk_arg2)
{
   ...
   if(channel == NULL)
   {
      OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR,"Channel doesnt exist");
      return;
   }
   OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_DEBUG,"Datapath ID %llx  and handle %llx",   channel->datapath->dpid, channel->dp_handle);
   if(!((channel->keep_alive_state & OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_READY_TO_SEND)
		   || (channel->keep_alive_state & OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_SENT)))
   {
      OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR," Error in channel KEEP alive state for dpid %llx", channel->datapath->dpid);
   }

   retval = of_send_echo_msg_on_channel(channel->dp_handle,channel->auxiliary_id);
   if(retval != OF_SUCCESS)
   {
      OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR," Error to send echo msg for dpid %llx", channel->datapath->dpid);
      return;
   }
...
void CNTLR_PKTPROC_FUNC of_transprt_check_keepalive_response_msg(void *cbk_arg1, void *cbk_arg2)
{
   ...
   OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG," uiKeepAliveCount : %d ",channel->uiKeepAliveCount);
   do
   {
      if( (!(channel->keep_alive_state & OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_RESP_RCVD)) &&
          (channel->keep_alive_state & OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_SENT)     &&
          (channel->uiKeepAliveCount > OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MAX_RETRIES)
            )
      {
      ...
      else
      {
	      if(channel->keep_alive_state & OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_RESP_RCVD)
	      {
		      ...
		      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"keep alive response recieved..updating keepalive count to zero");
	      }
      }
   }while(0);
