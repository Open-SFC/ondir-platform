...
int32_t unix_dm_tcp_transport_init (of_transport_proto_properties_t *transport_proto)
{
   ...
   do
   {
      ...
      transport_proto->fd = socket(AF_UNIX, SOCK_STREAM,0);
      ...
      /* Bind socket to the specified port*/
      {
         ...
         strncpy(local_addr.sun_path,unix_dm_tcp_proto_details_p->path_name,sizeof(local_addr.sun_path));
         ret_val = bind(transport_proto->fd, (struct sockaddr *)&local_addr, strlen(local_addr.sun_path) + sizeof(local_addr.sun_family));
      ...
      OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR,"listening on :%s",unix_dm_tcp_proto_details_p->path_name);
      ...
      void **opaque_pp)
{
   ...
   do
   {
      ...
      OF_LOG_MSG(OF_LOG_XPRT_PROTO, OF_LOG_ERROR, "fd:%d,lenght:%d",fd,length); 
