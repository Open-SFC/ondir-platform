...
#define EPOLL_EVENT_RCVD 5

/*Keep alive timer*/
#define OF_TRNSPRT_KEEP_ALIVE_TIME_PERIOD  30000000

#define OF_TRNSPRT_KEEP_ALIVE_RESP_WAIT_TIME  15000000
...
typedef struct  cntlr_channel_info_s
{
	...
	int32_t id; 
	uint8_t  		         auxiliary_id; /** Auxiliary ID of the channel opened from data_path*/ 
...
int32_t of_process_handshake_msg(cntlr_channel_info_t        *channel);

void
of_register_write_event_to_epoll_fd(cntlr_channel_info_t *channel);
int32_t 
CNTLR_PKTPROC_DATA cntlr_send_epoll_write_event_2_xprt_conn(cntlr_channel_info_t *channel);
...
#define OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_READY_TO_SEND  (1)
#define OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_SENT        (2)
#define OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_RESP_RCVD   (4)
#define OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MAX_RETRIES 3

/* * * * * * * * * * Channel Alive states* * * * * * * * * */
#define OF_TRNSPRT_CHN_SENT_HELLO_TO_DP                   (0)
...
{\
	...
	{\
		...
		if (conn_info) \
		{\
                OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"Going through AUX channel");\
