...
extern struct nlmsghdr *h;
extern struct sockaddr_nl *snl;
...
typedef int32_t (*nl_sync_fn)(uint64_t dp_id, uint32_t ns_id, uint32_t sock_fd);
...
struct ip4_unicast_lim_stats
{
  uint32_t  nl_total_events_received_count;
  uint32_t  nl_error_events_received_count;
  uint32_t  nl_add_events_count;
  uint32_t  nl_del_events_count;
  uint32_t  nl_add_events_fail_count;
  uint32_t  nl_del_events_fail_count;
  uint32_t  pkt_to_peth_count;
  uint32_t  pkt_from_peth_count;
  uint32_t  pkt_to_dp_count;
  uint32_t  pkt_from_dp_count;
};
...
int32_t listener_register_cbfn(uint32_t groups, int32_t priority, rtl_cbfunc cbfunc, void *cbarg_p, nl_sync_fn syncfunc );
...
#if (!defined CONFIG_SDK_VER_1_5 && !defined CONFIG_SDK_VER_1_6)
