...
struct dprm_datapath_entry
{
  ...
  struct ofp_async_config   async_msg_db_info;
  /** ID of the datapath */
  //uint64_t dpid;
...
int32_t cntlr_register_asynchronous_event_hook(uint64_t               domain_handle,
    ...
    void                  *cbk_arg2);
	int32_t
cntlr_deregister_asynchronous_event_hook(uint64_t               domain_handle,
		uint8_t                event_type,
		uint8_t                priority);
...
void dprm_free_datapath_channels(struct dprm_datapath_entry  *datapath_p);
int32_t of_add_new_role_to_dp(uint64_t datapath_handle,  uint32_t controller_role);
...
inline int32_t dprm_dp_table_init(uint8_t   no_of_tables,  void  **tables_p);
