...
#if 0
int32_t ipfwdapp_init();

int32_t iface_mod_init();

int32_t iface_mod_init();

int32_t ip_addr_init();


//void netlink_read_complete_rtmsg(struct rtl_ns_and_sock_map_entry *map_entry_p);

int32_t ipfwd_rt_rule_sync_init(int32_t ns_id, uint64_t dp_id);
...
int32_t iface_get_mtu_from_ifid(uint32_t ns_id, uint32_t ifid, uint32_t *mtu);
