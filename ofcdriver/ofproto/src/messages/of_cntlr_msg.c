...
/* PROTOTYPE_START   ****************************************************/
extern int32_t of_queue_frame_response(char *msg,struct ofl_queue_config *queue_config_info, uint32_t msg_length);
int32_t of_flush_msg_buf(cntlr_channel_info_t     *channel, uint16_t length);
void
of_process_queue_config_reply(cntlr_channel_info_t       *p_channel,
...
int32_t of_meter_frame_response(char *msg,
                                ...
                                uint32_t msg_length);
int32_t of_meter_frame_stats_response(char *msg,
                                struct ofi_meter_stats_info *meter_entry_p,
                                uint32_t msg_length);
...
#define QUEUE_GET_CONFIG_REPLY_CBK of_process_queue_config_reply
               ...
               case OFPT_PACKET_IN:
                  {
                     ...
                     of_alloc_pkt_mbuf_and_set_of_msg(mbuf, channel->msg_buf,OFPT_PACKET_IN, sizeof( struct ofp_packet_in), (ntohs(msg_hdr_p->length)- sizeof( struct ofp_packet_in)),flags, status);
                     if(status == FALSE)
                     {
    ...
    if(channel->msg_state == OF_CHN_START_READING_MSG)
    {
      ...
      if(p_msg == NULL)
      {
        ...
        if( of_drain_out_socket_buf(channel, ntohs(msg_hdr_p->length) - sizeof(struct ofp_header)) != OF_SUCCESS)
        {
          ...
          printf("\n of_alloc_and_read_message_from_dp -- of_drain_out_socket_buf \r\n");
        }
    ...
    struct ofp_header          *p_msg_hdr)
{
  ...
  int32_t         retval = OF_SUCCESS;
  struct pkt_mbuf *pkt_mbuf_p = NULL;
  uint8_t iphdr_len=0;
  ...
  do
  {
    ...
    * same can be used as output channel  for sendng packet out msg*/
    ...
    CNTLR_PKT_IN_DATA(p_datapath->dpid,pkt_in_msg);
...
#if 0
      if ( pktbuf_cbks ){
      pkt_mbuf_p = (struct pkt_mbuf *) pktmbuf_get_mbuf_ptr_by_msg_ptr(p_channel->msg_buf);

      /* updating nw hdr pointers */
      pkt_mbuf_p->data.ptr = pkt_in_msg.packet;
      ...
      pkt_mbuf_p->of_table_id =  pkt_in_msg.table_id;
      pkt_mbuf_p->count =  pkt_in_msg.packet_length;
      }
#endif
    				...
    				struct ofp_header          *p_msg_hdr)
{
 ...
 // uint8_t         type=0, sub_type;
  ...
  do
  {
	  ...
	  {
		  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,
				  "%s:%d Error - not a OFPT_ERROR message \r\n", __FUNCTION__,__LINE__);
                  OF_TRNSPRT_CHN_RESET_2_READ_NEW_MSG(p_channel);
		  return;
	  }
	  ...
	  if (p_msg_hdr->type == OFPT_ERROR)
	  {
		  ...
		  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, " OFPT_ERROR: \r\n", error_msg.type,  error_msg.code_or_exp_type);
          ...
          if(p_channel->msg_buf != NULL)
          {
                  p_channel->msg_buf->desc.free_cbk(p_channel->msg_buf);
                  p_channel->msg_buf = NULL;
          }
          if(CNTLR_LIKELY(p_channel->msg_state == OF_CHN_RCVD_COMPLETE_MSG))
          {
                  OF_TRNSPRT_CHN_RESET_2_READ_NEW_MSG(p_channel);
                  ...
                  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR, "ERROR: returned with msg state = %u \r\n", p_channel->msg_state);
...
#if 0
      msg = (struct of_msg *)p_channel->msg_buf;
      if(msg != NULL)
      {
        OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "freeing bind mod error msg");
    ...
    uint8_t                     more_data)
{
  ...
  int32_t                    status = OF_SUCCESS,retval;
  uint16_t   bytes_to_read;
  uint16_t   read_bytes=0;
  uint16_t   length=0;
  char *buf;
  ...
  for(meter_no = 1;(( meter_no <= no_of_meters) && (( bytes_to_read - read_bytes) >= sizeof(struct ofp_meter_stats)));meter_no++)
  {
    ...
    if (length > 0)
    {
	    ...
	    retval= of_meter_frame_stats_response(buf, &meter_stats_local, length);
	    if(retval != OF_SUCCESS)
	    {
		    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"of_meter_frame_response failed");
	    }
    ...
    uint8_t                     more_data)
{
  ...
  for(flowentry_no = 1; ((flowentry_no <= no_of_flowstats) && (( bytes_to_read - read_bytes) >= sizeof(struct ofp_flow_stats)));flowentry_no++)
  {
    ...
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"flow_stats_read->length=%ld  len_rcvd =%d",flow_stats_read->length, (sizeof( struct ofp_flow_stats) - sizeof(struct ofp_match)));
    ...
    struct ofp_header          *p_msg_hdr)
{
  ...
  if( p_msg_hdr->type  == OFPT_MULTIPART_REPLY )
  {
    ...
    printf("\n [ERROR] Flushing %d bytes from TCP Buffer., controller will not be able to read proper messages\r\n",
                    (ntohs(p_msg_hdr->length) - sizeof(struct ofp_header)) );
      }
      return;
    }
...
void CNTLR_PKTPROC_FUNC of_echo_req_handler(cntlr_channel_info_t       *channel,
    					    ...
    					    struct ofp_header          *p_msg_hdr)
{
  struct dprm_domain_entry   *p_dp_domain=NULL ;
  ...
  if(p_datapath  == NULL)
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR, " channel disconnected.");
//    return;
  }

  p_dp_domain = p_datapath->domain;
  if(p_dp_domain == NULL)
  {  
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"****DOMAIN IS NULL *****");
  //  return;
  }
  if(p_dp_domain->symmetric_msg_handle[msg_id].callback == NULL)
  {
     OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"****CALLBACK IS NULL *****");
    // return;
  }
    ...
    struct ofp_header          *p_msg_hdr )
{
  ...
  cntlr_del_xtn_with_rec(p_xtn_rec);
          ...
          if(CNTLR_LIKELY(channel->msg_state == OF_CHN_RCVD_COMPLETE_MSG))
          {
                  OF_TRNSPRT_CHN_RESET_2_READ_NEW_MSG(channel);
          }
          else
    ...
    struct ofp_header    *p_msg_hdr)
{
  ...
  if (msg_handle != NULL)
  {
	...
	//channel->keep_alive_state = OF_TRNSPRT_CHN_CONN_KEEP_ALIVE_MSG_READY_TO_SEND;
	channel->uiKeepAliveCount=0;
      }
    msg_handle(channel,channel->datapath,p_msg_hdr);
#if 1
    if(unlikely(OF_CHN_START_READING_MSG == channel->msg_state))
    {
      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"channel state is in OF_CHN_START_READING_MSG while returning to epoll  \r\n");
    }
    else if (unlikely(OF_CHN_RCVD_COMPLETE_MSG == channel->msg_state))
    {
      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"channel state is in OF_CHN_RCVD_COMPLETE_MSG while returning to epoll \r\n");
    }
    else if (unlikely(OF_CHN_DRAIN_OUT_COMPLETE_MSG == channel->msg_state))
    {
      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"channel state is in OF_CHN_DRAIN_OUT_COMPLETE_MSG while returning to epoll  \r\n");
    }
#endif
  }
...
int32_t of_frame_instruction_response(char *msg, struct ofi_instruction **instruction_entry_pp, uint32_t *length)
{
  ...
  instruction_entry_p->len=ntohs(instruction_read_p->length);
    ...
    struct ofp_header          *p_msg_hdr)
{
 cntlr_transactn_rec_t    *p_xtn_rec=NULL;
  int32_t                   retval = OF_SUCCESS;
  struct dprm_domain_entry *p_dp_domain = p_datapath->domain;
  struct ofp_queue_get_config_reply *queue_cnfg_reply;
  struct ofl_queue_config queue_config_info;
  uint32_t len=0;
  ...
  len = ntohs(p_msg_hdr->length) - sizeof(struct ofp_queue_get_config_reply);
  queue_cnfg_reply = (struct ofp_queue_get_config_reply *)(p_channel->msg_buf->desc.start_of_data);
  queue_config_info.port = ntohl(queue_cnfg_reply->port);

  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"port 0x%x\n",ntohl(queue_cnfg_reply->port));

   retval = of_queue_frame_response((char *)queue_cnfg_reply,&queue_config_info,len);
        if(retval == OF_FAILURE)
    {
      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"Error in framing queue response \r\n");
      goto returnfailure;
    }
  retval = cntlr_get_xtn_details(ntohl(p_msg_hdr->xid),
      p_datapath->datapath_id,
...
if(p_xtn_rec->callback_fn != NULL){
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"calling callback");
    ((of_queue_config_response_cbk_fn)(p_xtn_rec->callback_fn))(controller_handle,
    p_dp_domain->domain_handle,
    p_datapath->datapath_handle,
    NULL,
    p_xtn_rec->app_pvt_arg1,
    p_xtn_rec->app_pvt_arg2,
    OF_RESPONSE_STATUS_SUCCESS, &queue_config_info);
 }
