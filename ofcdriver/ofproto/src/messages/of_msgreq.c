...
int32_t cntlr_send_swinfo_desc_request_after_feature_reply(uint64_t datapath_handle)
{
  ...
  do  
  {
    ...
    retval =  of_get_switch_info_through_channel(msg,
        datapath_handle,
        (of_switch_info_cbk_fn)of_switch_info_reply_cbk_fn_after_feature_reply,
        NULL,
        NULL,
        NULL);
    if(retval != CNTLR_SUCCESS)
...
int32_t cntlr_send_meter_statistics_request(uint64_t datapath_handle,uint32_t  meter_id)
{
  ...
  do
  {
    ...
    retval =  of_get_meter_statistics(msg,
        datapath_handle,
        meter_id,
        (of_meter_stats_cbk_fn)of_meter_stats_reply_cbk_fn,
        NULL,
        NULL);
    if(retval != CNTLR_SUCCESS)
    {
    ...
    uint8_t   more_meters)
{
  ...
  if (meter_stats)
  {
    OF_LIST_REMOVE_HEAD_AND_SCAN(meter_stats->meter_band_stats, band_entry_p,struct ofi_meter_band_stats *,OF_METER_BAND_STATS_LISTNODE_OFFSET)
    {
      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"deleting band");
    }
  }
