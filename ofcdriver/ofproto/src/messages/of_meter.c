...
int32_t of_meter_frame_message_request(struct of_msg *msg, struct ofi_meter_rec_info *meter_entry_p)
{
  ...
  int32_t status=OFU_APPEND_BAND_SUCCESS;
  ...
  { 
    ...
    OF_LIST_SCAN(meter_entry_p->meter_band_list, band_entry_p, struct ofi_meter_band *,OF_METER_BAND_LISTNODE_OFFSET)
    {
	  ...
	  if(band_entry_p->rate == 0){
          ...
          ofu_append_band_to_meter_type_drop(msg, band_entry_p->rate,band_entry_p->burst_size);
          break;
	  ...
	  if(band_entry_p->rate == 0 || band_entry_p->prec_level == 0){
          ...
          ofu_append_band_to_meter_type_dscp_remark(msg, band_entry_p->rate, band_entry_p->prec_level, band_entry_p->burst_size);
          break;
...
int32_t of_meter_frame_stats_response(char *msg, struct ofi_meter_stats_info *meter_stats_p, uint32_t msg_length)
{
  struct ofi_meter_band_stats *band_stats_p;
  uint32_t band_stats_len;
  uint16_t offset;
  struct ofp_meter_band_stats *band_read_p;
  offset = OF_METER_BAND_STATS_LISTNODE_OFFSET;

  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"frame response msg_length %d", msg_length);

  OF_LIST_INIT(meter_stats_p->meter_band_stats, NULL);
  while (msg_length > 0 )
  {
    band_stats_len=0;
    band_read_p=(struct ofp_meter_band_stats *)msg;
    band_stats_p=(struct ofi_meter_band_stats *)calloc(1,sizeof(struct ofi_meter_band_stats));
    if(band_stats_p)
    {
      band_stats_p->packet_count = ntohll(band_read_p->packet_band_count);
      band_stats_p->byte_count = ntohll(band_read_p->byte_band_count);
      band_stats_len = sizeof(struct ofp_meter_band_stats);
    }
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"band_stats_len %d", band_stats_len);
    OF_APPEND_NODE_TO_LIST(meter_stats_p->meter_band_stats, band_stats_p,offset);
    msg += band_stats_len;
    msg_length -= band_stats_len;
  }
  meter_stats_p->no_of_bands = OF_LIST_COUNT(meter_stats_p->meter_band_stats);
  return OF_SUCCESS;
}
