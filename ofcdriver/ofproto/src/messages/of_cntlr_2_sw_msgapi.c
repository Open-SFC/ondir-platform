      ...
      uint32_t              *xid)
{
   ...
   struct ofp_meter_multipart_request  *meter_desc_req;
   struct dprm_datapath_entry   *datapath;
   cntlr_conn_node_saferef_t conn_safe_ref;
   cntlr_conn_table_t        *conn_table;
   uint8_t  send_status = TRUE;
   ...
   do
   {
      ...
      meter_desc_req = (struct ofp_meter_multipart_request*)(meter_config_req_multipart_msg->body);

      meter_desc_req->meter_id  = htonl(meter_id);
...
{   
   ...
   {
     OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"of_cntrlr_send_request_message successful");
   }    
      ...
      void *clbk_arg2)
{
   ...
   do
   {
      ...
      of_async_config_msg = (struct ofp_header *)(msg->desc.start_of_data);
