...
ofu_start_setting_match_field_values(struct of_msg *msg)
{
  ...
  msg_desc = &msg->desc;
  match = (struct ofp_match *)msg_desc->utils_ptr;
...
inline void ofu_set_template_match_field(uint8_t *tmatch_field, 
                                         uint32_t src_oxmid, 
                                         uint32_t dst_oxmid,
                                         uint16_t src_offset, 
                                         uint16_t dst_offset, 
                                         uint16_t nbits)
{
  struct ofp_oxm_template *tmpl_match = \
  (struct ofp_oxm_template *)tmatch_field;

  tmpl_match->oxm_header = htonl(OXM_OF_TEMPLATE);
  tmpl_match->src_oxm_id = htonl(src_oxmid);
  tmpl_match->dst_oxm_id = htonl(dst_oxmid);
  tmpl_match->src_offset = htons(src_offset);
  tmpl_match->dst_offset = htons(dst_offset);
  tmpl_match->n_bits     = htons(nbits);
}
#endif
...
#ifdef FSLX_COMMON_AND_L3_EXT_SUPPORT
int32_t 
ofu_push_flow_mod_action(struct of_msg *msg,
                         uint64_t cookie, 
...
{
  struct of_msg_descriptor *msg_desc;
  struct  ofp_action_flow_mod *flowmod_action;
  ...
  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, " match = %p match_len = %d inst=%p instlen=%d ",match_fields, mf_len, instructions, inst_len);


  action_len = ((OFU_FLOWMOD_ACTION_LEN - 4) + mf_len + inst_len);

  if( (msg_desc->ptr1+msg_desc->length1 + action_len) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  flowmod_action = (struct  ofp_action_flow_mod *)(msg_desc->ptr3+msg_desc->length3);
  flowmod_action->type    = htons(OFPAT_FLOW_MOD);
  flowmod_action->len     = htons(action_len);

  /* fill ofp header */
  flow_mod = (struct ofp_flow_mod *)(&flowmod_action->flow_mod);
  flow_mod->header.version = OFP_VERSION;
  flow_mod->header.type    = OFPT_FLOW_MOD;
  flow_mod->header.length  = htons(((sizeof(struct  ofp_flow_mod) - 4) + mf_len + inst_len));
  flow_mod->header.xid     = 0;

  /* fill flow-mod fixed information */
  ...
  /* fill flow-mod match fileds */
  flow_mod->match.type   = htons(OFPMT_OXM);
  flow_mod->match.length = htons(((sizeof(struct ofp_match) - 4) + mf_len));
   
  memcpy(flow_mod->match.oxm_fields, match_fields, mf_len); 
  insptr = (uint8_t *)(flow_mod->match.oxm_fields + mf_len);
  
  /* fill flow-mod instructions */
  memcpy(insptr, instructions, inst_len);

  /* update total action length */
  msg_desc->length3 += action_len;

  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,
  "msg->desc.ptr1 %p length1 %d msg->desc.ptr2 %p msg->desc.length2 %d msg->desc.ptr3 %p msg->desc.length3 %d msgdec.datalen %d ",
   msg->desc.ptr1, msg->desc.length1, msg->desc.ptr2, msg->desc.length2,msg->desc.ptr3, msg->desc.length3, msg->desc.data_len);
    ...
    uint32_t  queue_id)
{
  ...
  msg_desc = &msg->desc;
	...
	OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG," buf loc %p %p queue_id %d",(msg_desc->ptr1+msg_desc->length1 + OFU_SET_QUEUE_ACTION_LEN),
			(msg->desc.start_of_data + msg_desc->buffer_len) ,queue_id);

	if( (msg_desc->ptr1+msg_desc->length1 + OFU_SET_QUEUE_ACTION_LEN) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
	...
	set_queue = (struct ofp_action_set_queue *)(msg_desc->ptr3+msg_desc->length3);
	set_queue->type    = htons(OFPAT_SET_QUEUE);
	set_queue->len     = htons(sizeof(struct ofp_action_set_queue));
	set_queue->queue_id    = htonl(queue_id);
	msg_desc->length3 += OFU_SET_QUEUE_ACTION_LEN;
  OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"msg->desc.ptr1 %p length1 %d msg->desc.ptr2 %p msg->desc.length2 %d msg->desc.ptr3 %p msg->desc.length3 %d msgdec.datalen %d ",msg->desc.ptr1, msg->desc.length1, msg->desc.ptr2, msg->desc.length2,msg->desc.ptr3, msg->desc.length3, msg->desc.data_len);
  return OFU_ACTION_PUSH_SUCCESS;
    ...
    uint8_t  ipv4_ttl)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_OUTPUT_ACTION_LEN) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
...
ofu_push_dec_ipv4_ttl_action(struct of_msg *msg)
{
  ...
  if((msg_desc->ptr3+msg_desc->length3 + OFU_DEC_IP_TTL_ACTION_LEN) >
      (msg->desc.start_of_data + msg_desc->buffer_len)) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
    ...
    uint32_t *in_port_number)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_IN_PORT_LABEL_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint32_t*)(set_field->field + OF_OXM_HDR_LEN) = htonl(*in_port_number);
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_IN_PORT_LABEL_FIELD_ACTION_LEN), 0, padding_bytes);   
    ...
    uint32_t *in_phy_port_no)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_PHY_PORT_LABEL_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint32_t*)(set_field->field + OF_OXM_HDR_LEN) = htonl(*in_phy_port_no);
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_PHY_PORT_LABEL_FIELD_ACTION_LEN), 0, padding_bytes);   
    ...
    uint32_t  *mpls_label)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_MPLS_LABEL_FIELD_ACTION_LEN+ padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint32_t*)(set_field->field + OF_OXM_HDR_LEN) = htonl(*mpls_label);
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_MPLS_LABEL_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint8_t  *mpls_tc)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_MPLS_TC_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *mpls_tc;
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_MPLS_TC_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint8_t  *mpls_bos_bits)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_MPLS_BOS_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *mpls_bos_bits;
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_MPLS_BOS_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint8_t  *pbb_isid)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_PBB_ISID_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *pbb_isid;
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_PBB_ISID_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint64_t      *tunnel_id)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_TUNNEL_ID_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint64_t*)(set_field->field + OF_OXM_HDR_LEN) = htonll(*tunnel_id);
  memset(((uint8_t *)set_field + OFU_SET_FIELD_WITH_TUNNEL_ID_FIELD_ACTION_LEN), 0, padding_bytes);   
    ...
    char *dest_mac_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_ETH_DST_MAC_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint32_t*)set_field->field = htonl(OXM_OF_ETH_DST);
  ...
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_ETH_DST_MAC_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t *source_mac_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_ETH_SRC_MAC_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
  ...
  *(uint32_t*)set_field->field = htonl(OXM_OF_ETH_SRC);
  ...
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_ETH_SRC_MAC_ACTION_LEN),0,padding_bytes);
    ...
    uint16_t      *eth_frame_type)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_ETH_TYPE_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len))
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ALL,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*eth_frame_type);
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_ETH_TYPE_ACTION_LEN),0,padding_bytes);   
    ...
    uint16_t      *vlan_id)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_VLAN_VID_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ALL,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*vlan_id);
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_VLAN_VID_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t       *vlan_priority_bits)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_VLAN_PCP_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ALL,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *vlan_priority_bits;
  memset( ((uint8_t *)set_field + OFU_SET_FIELD_WITH_VLAN_PCP_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t      *ip_dscp_bits)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_IP_DSCP_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *ip_dscp_bits;
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IP_DSCP_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t      *ip_ecn_bits)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_IP_ECN_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *ip_ecn_bits;
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IP_ECN_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t      *ip_protocol)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_IP_PROTO_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *ip_protocol;
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IP_PROTO_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    ipv4_addr_t   *ipv4_addr)
{
 ...
 if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_IPV4_SRC_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)(set_field->field + OF_OXM_HDR_LEN) = htonl(*ipv4_addr);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV4_SRC_FIELD_ACTION_LEN),0,padding_bytes);
...

{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_IPV6_SRC_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)set_field->field   = htonl(OXM_OF_IPV6_SRC);
  ...
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV6_SRC_FIELD_ACTION_LEN),0,padding_bytes);
...

{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_IPV6_DST_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)set_field->field   = htonl(OXM_OF_IPV6_DST);
  memcpy((ipv6_addr_t *)(set_field->field + OF_OXM_HDR_LEN) , &(ipv6_addr->ipv6Addr32), sizeof(ipv6_addr->ipv6Addr32)); 
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV6_DST_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    uint32_t  *flow_label)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_IPV6_FLABEL_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t *)((uint8_t*)set_field->field + OF_OXM_HDR_LEN) = htonl(*flow_label); 
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV6_FLABEL_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t  *icmpv6_type)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_ICMPV6_TYPE_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   ) 
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint8_t *)(set_field->field + OF_OXM_HDR_LEN) = *icmpv6_type; 
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_ICMPV6_TYPE_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t  *icmpv6_code)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_ICMPV6_CODE_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint8_t *)(set_field->field + OF_OXM_HDR_LEN) = *icmpv6_code;
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_ICMPV6_CODE_FIELD_ACTION_LEN),0,padding_bytes);
...
{  
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_IPV6_ND_TARGET_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)set_field->field   = htonl(OXM_OF_IPV6_ND_TARGET);
  memcpy((ipv6_addr_t *)(set_field->field + OF_OXM_HDR_LEN) , &(ipv6_addr->ipv6Addr32), sizeof(ipv6_addr->ipv6Addr32)); 
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV6_ND_TARGET_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t  *hw_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_IPV6_ND_SLL_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ALL,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint32_t*)set_field->field = htonl(OXM_OF_IPV6_ND_SLL);
  ...
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV6_ND_SLL_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    uint8_t  *hw_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_IPV6_ND_DLL_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ALL,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint32_t*)set_field->field = htonl(OXM_OF_IPV6_ND_TLL);
  memcpy( (set_field->field + OF_OXM_HDR_LEN), hw_addr, 6);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV6_ND_DLL_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    uint16_t  *ipv6_exthdr_pseudo_fields)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3 + OFU_SET_FIELD_WITH_IPV6_EXTHDR_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ALL,"%s %d :Length of buffer is not sufficient to add data\r\n",
        __FUNCTION__,__LINE__);
  ...
  *(uint16_t *)(set_field->field + OF_OXM_HDR_LEN) = htons(*ipv6_exthdr_pseudo_fields);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV6_EXTHDR_FIELD_ACTION_LEN),0,padding_bytes);
    ...
    ipv4_addr_t   *ipv4_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_IPV4_DST_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)(set_field->field + OF_OXM_HDR_LEN) = htonl(*ipv4_addr);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_IPV4_DST_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint16_t  *port_number)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_TCP_SRC_PORT_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*port_number);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_TCP_SRC_PORT_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint16_t  *port_number)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_TCP_DST_PORT_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*port_number);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_TCP_DST_PORT_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint16_t  *port_number)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_UDP_SRC_PORT_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"%s %d :Length of buffer is not sufficient to add data\r\n",__FUNCTION__,__LINE__);
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*port_number);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_UDP_SRC_PORT_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint16_t  *port_number)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_UDP_DST_PORT_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*port_number);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_UDP_DST_PORT_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint16_t  *port_number)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_SCTP_SRC_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*port_number);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_SCTP_SRC_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint16_t  *port_number)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_SCTP_DST_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*port_number);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_SCTP_DST_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint8_t  *icmpv4_type)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_ICPMPV4_TYPE_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *icmpv4_type;
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_ICPMPV4_TYPE_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint8_t  *icmpv4_code)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_ICPMPV4_CODE_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint8_t*)(set_field->field + OF_OXM_HDR_LEN) = *icmpv4_code;
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_ICPMPV4_CODE_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint16_t  *arp_opcode)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+OFU_SET_FIELD_WITH_ARP_OP_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint16_t*)(set_field->field + OF_OXM_HDR_LEN) = htons(*arp_opcode);
  memset( ((uint8_t*)set_field +  OFU_SET_FIELD_WITH_ARP_OP_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    ipv4_addr_t  *ipv4_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+ OFU_SET_FIELD_WITH_ARP_SPA_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)(set_field->field +  OF_OXM_HDR_LEN) = htonl(*ipv4_addr);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_ARP_SPA_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    ipv4_addr_t  *ipv4_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+ OFU_SET_FIELD_WITH_ARP_TPA_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)(set_field->field + OF_OXM_HDR_LEN ) = htonl(*ipv4_addr);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_ARP_TPA_FIELD_ACTION_LEN),0,padding_bytes);   
    ...
    uint8_t  *hw_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+ OFU_SET_FIELD_WITH_ARP_SHA_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)set_field->field   = htonl(OXM_OF_ARP_SHA);
  memcpy( (set_field->field + OF_OXM_HDR_LEN), hw_addr, 6);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_ARP_SHA_FIELD_ACTION_LEN), 0, padding_bytes);
    ...
    uint8_t  *hw_addr)
{
  ...
  if( (msg_desc->ptr3+msg_desc->length3+ OFU_SET_FIELD_WITH_ARP_DHA_FIELD_ACTION_LEN + padding_bytes) >
      (msg->desc.start_of_data + msg_desc->buffer_len)   )
  {
    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_WARN,"Length of buffer is not sufficient to add data\r\n");
    return OFU_NO_ROOM_IN_BUFFER;
  ...
  *(uint32_t*)set_field->field   = htonl(OXM_OF_ARP_THA);
  memcpy( (set_field->field + OF_OXM_HDR_LEN), hw_addr, 6);
  memset( ((uint8_t*)set_field + OFU_SET_FIELD_WITH_ARP_DHA_FIELD_ACTION_LEN), 0, padding_bytes);
    ...
    uint32_t       burst_value)
{
  ...
  msg_desc->length2 += OFU_METER_BAND_LEN;
...
int32_t of_get_flow_rem_msg_match_fields( struct ofl_flow_removed  *flow_rem_msg,   struct ofl_oxm_match_fields *oxm_match_fields_p, uint64_t *flags_p)
{
   ...
   uint32_t length;
   uint32_t field;
   uint32_t oxm_class, tlv_len;
   uint8_t mask;

   OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"Reading Match fields from Flow Removed Message");

   tlv = (of_match_tlv_t *) (flow_rem_msg->match_fields);; /* first TLV */

   length=flow_rem_msg->match_field_len;
   OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"length %d",length);
   while(length > 0)
   {
      field = tlv->oxm_field;           /* Look at field identifier */
      mask = tlv->has_mask;
      oxm_class = tlv->oxm_class;
      OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"class %x field %d length %d ", oxm_class, field,length);
      if (oxm_class == OFPXMC_OPENFLOW_BASIC)
      {
         switch (field)
         {
            ...
            case OFPXMT_OFB_ICMPV6_CODE:
               break;
            default:
               OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"Unknown match field %d", field);
               break;
         }
      }
      ...
      {
         OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR,"invalid OXM_CLASS=%x",oxm_class);
      }
