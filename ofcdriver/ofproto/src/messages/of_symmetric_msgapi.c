...
extern inline dprm_dp_channel_entry_t*
of_get_dp_channel(uint64_t  dp_handle, uint8_t  auxiliary_id);
                                  ...
                                  )
{
     ...
     dprm_dp_channel_entry_t      *aux_channel;
     ...
     aux_id = *((uint8_t *)cbk_arg1); 
     OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG,"Aux Id : %d",aux_id);
     ...
     do
     {
        ...
        if(aux_id != 0)
        {  
           aux_channel = of_get_dp_channel(datapath_handle,aux_id);
           conn_safe_ref = aux_channel->conn_safe_ref;
           conn_table    = aux_channel->conn_table;
        }
        else
        {
