...
struct pkt_mbuf
{
        ...
        uint32_t                        of_msg_offset; /* offset to struct of_msg in skip area */
        uint8_t  of_app_type; /* enum of_appl_type */
        uint8_t  of_app_subtype; /* flags is to used by the application */
        uint8_t  of_app_flags; /* flags is to used by the application */
        int32_t  os_if_index; 
        uint32_t of_table_id;
	
	/* APPlication Private Data , Added for FWNAT module usage */
        uint8_t                         cb[12];
        uint32_t                        dst_ip;
        uint32_t                        spi;
        uchar8_t                        l3_proto;
...
*/
#define pkt_mbuf_hw_area_ptr(mbuf) ((uchar8_t*) ((unsigned long)(mbuf) - pkt_mbuf_hw_area_size()))
...
#define pkt_mbuf_skip_area_ptr(mbuf)  ( (uchar8_t*) ((uint32_t)(mbuf) + sizeof(struct pkt_mbuf)) )
 \n<b>Description:</b>\n
*/
#define pkt_mbuf_skip_area_ptr(mbuf)  ( (uchar8_t*) ((unsigned long)(mbuf) + sizeof(struct pkt_mbuf)) )
...
\n<b>Description:</b>\n
 ICMP Type
*/
#define pkt_mbuf_icmp_type(mbuf)      ((mbuf)->xport_tuple.icmp_type)
...
/** \ingroup PBUFMACROS
*/
#define PKT_MBUF_ICMP_CODE_OFFSET    1
...
#define pkt_mbuf_icmp_code_ptr(x)   \
           (pkt_mbuf_nw_hdr_ptr(x) + pkt_mbuf_ipv4_hdr_len(x) + PKT_MBUF_ICMP_CODE_OFFSET)
...
#define pkt_mbuf_icmpcode(x)     (*(pkt_mbuf_icmp_code_ptr(x)))
/* UDP related macros */
/** \ingroup PBUFMACROS
...
{\
  ...
  {\
   ...
   addr = (unsigned long)mbuf->data.ptr + offset ;\
   data_start = (unsigned long)(mbuf->data.ptr);\
   data_end   = (unsigned long)(mbuf->data.ptr) + (mbuf->count);\
  ...
  {\
   ...
   do \
   {\
     addr = (unsigned long)buf->data.ptr;\
...
{\
  ...
  {\
    ...
    while(1)\
    {\
      start = (unsigned long)buf->data.ptr;\
	 ...
	 (addr >  start + buf->count))\
      ...
      {\
       offset +=  (addr - ((unsigned long)buf->data.ptr));\
       break;\
      }\
    }\
  }\
...
{\
  ...
  {\
   ...
   data_end   = (unsigned long)(mbuf->start_of_data) + pkt_mbuf_max_data_size(mbuf);\
    if((addr < data_start) ||   (addr > data_end ))\
    {\
      offset = PSP_FAILURE;\
      pkt_mbuf_debug("%s:%d Panic!!! Offset is going beyond valid data!!!   \r\n",__FUNCTION__,__LINE__);\
  ...
  {\
    buf = mbuf;\
    start = (unsigned long)buf->start_of_data;\
    while(1)\
    {\
      if((addr < start) ||\
	 (addr >  start + pkt_mbuf_max_data_size(buf)))	\
      ...
      {\
       offset +=  (addr - ((unsigned long)buf->start_of_data));\
       break;\
      }\
    }\
  }\
...
{\
  ...
  {\
    unsigned long addr,data_start,data_end ;\
    addr = (unsigned long)data_ref.ptr + num_bytes ;\
    data_start = (unsigned long)(mbuf->start_of_data);\
...
{\
  ...
  { \
   ...
   addr      = (unsigned long)data_ref.ptr + num_bytes ; \
   data_start = (unsigned long)(mbuf->data.ptr);\
   data_end   = (unsigned long)(mbuf->data.ptr) + pkt_mbuf_max_data_size(mbuf) - 1 ;\
   \
    if((addr >= data_start) && (addr <=  data_end))    \
    {\
    ...
    {\
      printf("%s:%d Panic!!! Data reference going beyond valid data!!!   \r\n",__FUNCTION__,__LINE__);\
      printf("\r\n Addr = 0x%lx, Data Begin = 0x%lx, Data End = 0x%lx    \r\n",addr,data_start,data_end);\
    }\
  }\
  else \
  {\
...
{\
  ...
  { \
   unsigned long addr,data_start,data_end ;\
   addr      = (unsigned long)data_ref.ptr + num_bytes -1 ; \
   data_start = (unsigned long)(mbuf->start_of_data);\
   data_end   = (unsigned long)(mbuf->data.ptr) + pkt_mbuf_max_data_size(mbuf) - 1;\
    if((addr >= data_start) && (addr <=  data_end))    \
    {\
...
{\
  struct pkt_mbuf *buf;\
  offset = 0;\
  unsigned long addr,start,max_data;\
  addr  = (unsigned long)data_ref.ptr;\
  \
  if(!(pkt_mbuf_is_sg_valid(mbuf)))\
  {\
  ...
  {\
    ...
    {\
	 ...
	 (addr >=  (start + max_data) ))\
      ...
      {\
       offset +=  (addr - ((unsigned long)buf->data.ptr));\
       /*offset +=  ( max_data - (  addr - ((unsigned long)buf->data.ptr)));*/\
       break;\
      }\
    }\
  }\
...
\n<b>Description:</b>\n
 This Macro gives data_ref at given offset from SoD
  ...
  <i> None </i>
*/
#define pkt_mbuf_get_data_data_ref_ad_sod_offset( mbuf, offset, data_ref) \
{\
  unsigned long addr;\
  if((pkt_mbuf_is_sg_valid(mbuf) != TRUE))\
  {\
   unsigned long addr,data_start,data_end ;\
   addr = (unsigned long)mbuf->start_of_data + offset ;\
   data_start = (unsigned long)(mbuf->start_of_data);\
   data_end   = (unsigned long)(mbuf->start_of_data) + pkt_mbuf_max_data_size(mbuf);\
   if((addr>=data_start) && (addr<data_end))    \
   {\
  ...
  {\
   ...
   struct pkt_mbuf *buf = mbuf;\
   num_bytes = offset; \
   addr = (unsigned long)buf->start_of_data;\
   do \
   {\
...
#define pkt_mbuf_alloc(a,b) ({\
     ...
     /* p = __pkt_mbuf_alloc(a,b);*/\
       OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "pkt_mbuf_alloc");\
...
#define pkt_mbuf_free(p) ({\
     ...
     /* __pkt_mbuf_free(p);*/\
       OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "pkt_mbuf_free");\
...
extern struct dll_queue *buf_queue;
...
#ifdef OF_PKT_MBUF_DEBUG_STATS
#define OF_PKT_MBUF_INC_ALLOC_CNT(a, mbuf)    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "pkt_mbuf_alloc size %d count %d ptr 0x%x", a,++pkt_mbuf_alloc_stats_g,mbuf);
#else
#define OF_PKT_MBUF_INC_ALLOC_CNT(a,mbuf)
#endif 



#ifdef OF_PKT_MBUF_DEBUG_STATS
#define OF_PKT_MBUF_INC_FREE_CNT(mbuf)    OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "pkt_mbuf_free count %d ptr 0x%x", ++pkt_mbuf_free_stats_g,mbuf);
#else
#define OF_PKT_MBUF_INC_FREE_CNT(mbuf)
#endif 

#define pktmbuf_get_mbuf_ptr_by_msg_ptr(msg)  ((uchar8_t*) (((unsigned long)(msg) -  msg->pkt_mbuf_offset ) - sizeof(struct pkt_mbuf)))
...
{\
   struct pkt_mbuf *mbuf = NULL;\
	...
	{\
		if ( pktbuf_cbks )\
		{\
			mbuf=(struct pkt_mbuf *)pktmbuf_get_mbuf_ptr_by_msg_ptr(msg); \
         		/*OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "new msg ptr 0x%x msg->pkt_mbuf_offset %d pkt_mbuf ptr 0x%x", msg, msg->pkt_mbuf_offset, mbuf);*/\
			     if(mbuf)\
			     {\
				     OF_LOG_MSG(OF_LOG_MOD, OF_LOG_DEBUG, "pkt_mbuf_free");\
				     pktbuf_cbks->pktbuf_free(mbuf);\
...
#define pktmbuf_get_msg_ptr_by_mbuf_data_ptr(mbuf, protosize) \
  ((struct of_msg *) (pkt_mbuf_data_ptr(mbuf) - protosize -  sizeof(struct of_msg)))


#define pktmbuf_get_msg_ptr_by_mbuf_ptr(mbuf) \
  ((struct of_msg *) ((uchar8_t *)(mbuf) + sizeof(struct pkt_mbuf) + (mbuf)->of_msg_offset))

#define of_alloc_set_msg_params(mbuf, msg, msg_type, totalsize)\
{\
           msg->pkt_mbuf_offset = (uchar8_t *)msg - pkt_mbuf_skip_area_ptr(mbuf);\
           mbuf->of_msg_offset= msg->pkt_mbuf_offset; \
	   msg->desc.type            = msg_type;\
	   msg->desc.buffer_len      = totalsize + ROUND_UP(totalsize);\
	   msg->desc.data_len        = 0;\
...
#define of_alloc_pkt_mbuf_and_set_of_msg(mbuf, msg, msg_type, protosize, datasize, flags, status)\
{\
	status = TRUE;\
	if ( pktbuf_cbks )\
	{\
		mbuf = pktbuf_cbks->pktbuf_alloc (datasize, flags);\
		...
		{\
		 	mbuf->of_msg_offset= pkt_mbuf_prepend_size() - protosize - sizeof(struct of_msg);\
			/*OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR, "pkt_mbuf ptr 0x%x dataptr 0x%x ofmsgoffset %d", mbuf,pkt_mbuf_data_ptr(mbuf),mbuf->of_msg_offset);*/\
                        msg = pktmbuf_get_msg_ptr_by_mbuf_ptr(mbuf);\
		        /*OF_LOG_MSG(OF_LOG_MOD, OF_LOG_ERROR, "msg ptr 0x%x", msg);*/\
                        of_alloc_set_msg_params(mbuf, msg, msg_type, (protosize + datasize));\
		}\
	}\
	else \
	{\
		msg=ofu_alloc_message(msg_type, (protosize + datasize));\
	}\
}
