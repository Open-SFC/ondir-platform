...
int peth_trigger_arp_request (char *ns_name, int32_t if_index,
                                uint32_t target_ip);
int32_t netdev_get_kernel_flags  (uint32_t nsid, uint32_t ifid, uint32_t *flags_p);
...
struct peth_net_dev_flags_ioctl_param {
	int32_t if_index;
	uint32_t flags;
};
...
enum peth_ioctl_cmd_nr{
	...
	PETH_IOCTL_CMD_NR_GET_NET_DEV_FLAGS,
...
#define PETH_IOCTL_GET_NET_DEVICE_FLAGS    _IOWR(0, PETH_IOCTL_CMD_NR_GET_NET_DEV_FLAGS, struct peth_net_dev_flags_ioctl_param)
