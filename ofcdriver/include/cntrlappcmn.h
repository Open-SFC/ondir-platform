...
#define IPFWD_APP_SELF_PKT_PRIORITY     1
...
#define OF_GET_IPSEC_ACTION_FROM_METADATA(meta_data, ipsec_action) \
{\
   ipsec_action = ((meta_data & 0x0000000000ff0000) >> 16);\
}
...
int32_t cntlr_configure_switch(uint64_t datapath_handle);
void cntlr_send_port_desc_request(uint64_t datapath_handle);
int32_t cntlr_send_swinfo_desc_request_after_feature_reply(uint64_t datapath_handle);
int32_t of_send_echo_msg_on_channel(uint64_t datapath_handle, uint8_t auxilaryID);
int32_t
