...
int32_t of_set_portmod_config(struct    of_msg *msg,
      uint64_t  datapath_handle, void     *clbk_command_arg1,
      void     *clbk_command_arg2,  struct ofp_port_mod *port_info);
...
int32_t cntlr_send_meter_config_request(uint64_t datapath_handle,uint32_t  meter_id);
...
/** \ingroup  Queue_Config
 * \struct ofl_queue_config
 * \brief queue Confiugration Reply message, all fields are in host byte order\n
 */
struct ofl_queue_config
{
  uint32_t port;
  uint16_t property;
  of_list_t queue_list;

};

struct ofl_queue{
of_list_node_t     next_queue;
uint32_t ofq_id;
uint32_t  cq_id;
};

#define OF_QUEUE_LISTNODE_OFFSET offsetof(struct ofl_queue, next_queue)
/** \ingroup Switch_Config
 * \typedef of_switch_config_response_cbk_fn
 * \brief Callback function to receive switch configuration reply.
 * <b>Description </b>\n
 ...
 *                                 get the configuration details of data_path.
 */
typedef void (*of_queue_config_response_cbk_fn)(uint64_t  controller_handle,
                                                uint64_t  domain_handle,
                                                uint64_t  datapath_handle,
						 struct of_msg *msg,
                                                void     *cbk_arg1,
