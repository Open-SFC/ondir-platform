...
int32_t cm_cli_interpret_command (/* volatile */struct cm_cli_session * cli_session_p, /*const*/ volatile char * line_p,volatile unsigned char is_tab_pressed_b)
{
   ...
   struct cm_cli_params params[CM_CLI_MAX_CMDPARAMS] = {};
   ...
   return_value = of_tok_line (tok_p, line_info_p, (int *) &count_i,&temp_tokens_a,
         (int *) &cursor_index_i, (int *) &cursor_offset_i);
   /* return 0 on success */
   if (return_value < 0)
   {
            ...
            if (!of_strcmp ((char*)tokens_a, cm_cli_default_cmd_list[i].command_name_c))
            {
               ...
               if(count_i > 1)
               {
                  ...
                  CM_DLLQ_SCAN (&cli_session_p->current_node_p->child_list, dll_q_node_p,UCMDllQNode_t *)
                  {
                     ...
                     if (of_strcmp ((char *) data_ele_node_p->node_name, tmp_str_a) == 0) //table name
                     {
         ...
         if (found_b == FALSE)
         {
            if(count_i == 1)
            {
               CM_DLLQ_SCAN (&cli_session_p->current_node_p->child_list, dll_q_node_p,UCMDllQNode_t *)
               {
                  ...
                  if (of_strcmp ((char *) data_ele_node_p->node_name, (char *)tokens_a) == 0)
                  {
                     found_b = TRUE;
                     if (data_ele_node_p->node_type == CM_DMNODE_TYPE_TABLE)
                     {
                        if(data_ele_node_p->system_command_b == TRUE)
                        {
                           ...
                           if(is_tab_pressed_b == TRUE)
                              cm_cli_display_instances_and_key_params ((char *) data_ele_node_p->node_name,TRUE);
                           else
                        ...
                        else
                        {
                            ...
                            * Table */
                           cm_cli_display_instances_and_key_params ((char *) data_ele_node_p->node_name,FALSE);
                        }
                     }
                     else if (data_ele_node_p->node_type == CM_DMNODE_TYPE_ANCHOR)
                     {
                        cm_cli_validate_and_change_dir((char *) data_ele_node_p->node_name);
                     }
                  }
               }
               if (found_b == FALSE)
            ...
            else
            {
               ...
               CM_DLLQ_SCAN (&(cli_session_p->current_node_p->child_list), dll_q_node_p,UCMDllQNode_t *)
               {
                  ...
                  if ((!of_strcmp (tmp_str_a, (char *) data_ele_node_p->node_name)) &&
...
char* cm_cli_get_context_prompt (edit_line * edit_line_ctx_p)
{
   ...
   if (cli_session_p->root_c[0] == '\0')
   {
      ...
      return_value = cm_dm_get_first_node_info (NULL, UCMDM_TRAVERSAL_TYPE_TOP_TO_BOTTOM,
            ...
            &node_info_p, (void *) &id_array_p, &len_ui);
      if ((return_value == OF_SUCCESS) && (node_info_p))
      {
         ...
         of_strcpy ((char *) cli_session_p->current_node_p->node_name, cli_session_p->root_c);
                                        ...
                                        unsigned char add_b, unsigned char *is_table_b)
{
   ...
   *is_table_b = FALSE;
   ...
   if (tmp_cli_session_p->current_node_p)
   {
      ...
      of_strncpy ((char *) tmp_cli_session_p->current_node_p->node_name, node_info_p->name_p,
            of_strlen ((char *) node_info_p->name_p));
      /****
       * Key Child Info we need to copy. 
       ****/
      if (node_info_p->element_attrib.element_type == CM_DMNODE_TYPE_TABLE)
      {
         if ( (of_strcmp((char *) cli_session_p->current_node_p->node_name, "vsg") == 0) && (dm_path_p[of_strlen(dm_path_p)-1] == '}'))
         {
            *is_table_b = FALSE;
         }
         else
         ...
         unsigned char add_b)
{
    ...
    */
   if( (of_strcmp((char *) current_node_p->node_name, "vsg") == 0) && (add_b == TRUE))
                              ...
                              struct cm_dm_node_info * node_info_p)
{
   ...
   /* copy name */
   of_strcpy ((char *) node_p->node_name, node_info_p->name_p);
   ...
   else
   {
      while (TRUE)
      {
         ...
         else
         {
            ...
            if ((node_info_p->element_attrib.element_type == CM_DMNODE_TYPE_TABLE) && 
                (node_info_p->element_attrib.global_trans_b != TRUE) && 
                ( of_strchr (token_a, '{') != NULL))
            {
   ...
   else
   {
      CM_DLLQ_SCAN (&cli_session_p->current_node_p->data_elements_list, dll_q_node_p, UCMDllQNode_t *)
      {
         data_ele_node_p = CM_DLLQ_LIST_MEMBER (dll_q_node_p, struct cm_cli_data_ele_node *,
               list_node);
         if (of_strcmp ((char *)data_ele_node_p->node_name, name_p) == 0)
         {
            match_found_b = TRUE;
            break;
         }
   ...
   if (match_found_b == TRUE)
   {
      ...
      switch (data_ele_node_p->data_attrib.data_type)
      {
         ...
         case CM_DATA_TYPE_INT64:
            ...
            sprintf(value_p,"%ld", (long int) val_num_i);
            of_free(nv_pair_p->value_p);
            nv_pair_p->value_p = value_p;
...
*/
      ...
      switch (data_ele_node_p->data_attrib.attrib_type)
      {
         ...
         case CM_DATA_ATTRIB_IPADDR_ENUM:
            ...
            {
               if (!strcmp
                     (value_p, (char *) data_ele_node_p->data_attrib.attr.ip_enum.ip_arr_a[cnt_ui]))
               {
                  match_found_b = TRUE;
               }
            }
   ...
   switch (data_ele_node_p->data_attrib.attrib_type)
   {
      ...
      case CM_DATA_ATTRIB_INT64_RANGE:
         sprintf (buff_a, " [ %ld to %ld ]",
               (long int) data_ele_node_p->data_attrib.attr.longint_range.start_value,
               (long int) data_ele_node_p->data_attrib.attr.longint_range.end_value);
         of_strcat(tmp_sort_buff[tmp_attrib_cnt_i],buff_a);
         break;
      case CM_DATA_ATTRIB_UINT64_RANGE:
         sprintf (buff_a, " [ %lu to %lu ]",
               (long unsigned int) data_ele_node_p->data_attrib.attr.ulongint_range.start_value,
               (long unsigned int) data_ele_node_p->data_attrib.attr.ulongint_range.end_value);
         of_strcat(tmp_sort_buff[tmp_attrib_cnt_i],buff_a);
         break;
      case CM_DATA_ATTRIB_INT64_ENUM:
         of_strcat(tmp_sort_buff[tmp_attrib_cnt_i]," { ");
         ...
         {
            sprintf (buff_a, "%ld",
                  (long int) data_ele_node_p->data_attrib.attr.longint_enum.array[cnt_ui]);
            if(cnt_ui > 0)
               of_strcat(tmp_sort_buff[tmp_attrib_cnt_i], CM_CLI_ENUM_SEPERATOR);
            of_strcat(tmp_sort_buff[tmp_attrib_cnt_i],buff_a);
         }
      ...
      case CM_DATA_ATTRIB_UINT64_ENUM:
         ...
         {
            sprintf (buff_a, "%lu",
                  (long unsigned int) data_ele_node_p->data_attrib.attr.ulongint_enum.array[cnt_ui]);
            if(cnt_ui > 0)
               of_strcat(tmp_sort_buff[tmp_attrib_cnt_i], CM_CLI_ENUM_SEPERATOR);
            of_strcat(tmp_sort_buff[tmp_attrib_cnt_i],buff_a);
         }
...
void cm_cli_display_config_session (struct cm_array_of_commands * cmd_array_p)
{
   ...
   for (ii = 0; ii < cmd_array_p->count_ui; ii++)
   {
      ...
      cm_cli_display_nv_pair (tmp_cmd_p->nv_pair_array.count_ui, 
            &tmp_cmd_p->nv_pair_array,tmp_cmd_p->dm_path_p, NULL, FALSE);
   }
}
...
int32_t cm_cli_user_login(unsigned char *user_name_p, unsigned char *password_p)
{
   ...
   if(cm_cli_get_password((unsigned char*) passwd_a)!= OF_SUCCESS)
   {
...
#else
   if((cm_pam_authenticate_user((char *) user_name_a, passwd_a, &role_info_p)) != OF_SUCCESS)
   { 
      cm_cli_puts(cli_session_p, "Authentication failed! Wrong User Name/Password.\n\r");
      if(role_info_p != NULL)
      {
   ...
   of_strcpy(cli_session_p->role_info.admin_name, (char *) user_name_a);
   if(role_info_p != NULL)
   {
      of_free(role_info_p);
      role_info_p = NULL;
...
{
   char *p, *q = (char *) user_name_p;

   cm_cli_puts(cli_session_p, "\n\rUser Name: ");
   if(!of_fgets((char *) user_name_p, CM_MAX_ADMINNAME_LEN, CM_CLI_STDIN))
   {
      return OF_FAILURE;
   }

   p = of_strchr((char *) user_name_p, '\n');
   if ( p !=  NULL)
   {
      user_name_p[p-q] = '\0';
   }
...
{
   of_strncpy((char *) password_p, getpass("Password: "), CM_MAX_ADMINNAME_LEN);
   return OF_SUCCESS;
}
...
int32_t cm_cli_get_server_ip(uint32_t *server_ip_p)
{
   ...
   char *p, *q;
   cm_cli_puts(cli_session_p, "\n\rDevice IP Address: ");
   if(!of_fgets((char *)ip_addr_a, CM_MAX_ADMINNAME_LEN, CM_CLI_STDIN))
   {
      //return NULL;
      return OF_FAILURE;
   }
   ...
   q= (char *)&ip_addr_a[0];
   p =  of_strchr((char *) ip_addr_a, '\n');
   if ( p !=  NULL)
   {
      ip_addr_a[p-q] = '\0';
   }

   if(cm_val_and_conv_aton((char *) ip_addr_a, &ip_address) == OF_FAILURE )
   {
      if(!of_strcmp((char *) ip_addr_a,""))
      {
         ip_address=0;
      }
      else
