                              ...
                              char *pInstPath, struct cm_je_config_session *pConfigSess)
{
  ...
  FILE * pFile;
  struct cm_array_of_nv_pair *array_of_nv_pairs_p = NULL;
                                     ...
                                     void * pTransportChannel)
{
  ...
  iDestFd = cm_file_open (iDestFd, (unsigned char *) pFullPath, UCMFILE_MODE_WRITE_APPEND);
  if(iDestFd < 0)  //CID 164
  {
    if (errno == ENOENT)
    {
      iDestFd = cm_file_create (iDestFd, (unsigned char *) pFullPath);
  ...
  sprintf (aBuffer, "\n\t\t<row>");
  cm_file_write (iDestFd, iDestFd, (unsigned char *) aBuffer, strlen (aBuffer));
        ...
        if (pTmpNodeInfo->element_attrib.non_config_leaf_node_b == FALSE)
        {
          if (strcmp(pKeys->nv_pairs[ii].name_p, pTmpNodeInfo->name_p)==0)
          {
            ...
            if (pHashValue != NULL)
            {
              ...
              cm_file_write (iDestFd, iDestFd, (unsigned char *) pBuffer, strlen(pBuffer));
              of_free(pBuffer);
            }
          }
        }
...
int32_t cm_ldsv_create_backup (char * pFileName, char * pDirPath)
{
  ...
  pBkpFile = (unsigned char*) of_calloc(1, uiLength+1);
  if (pBkpFile == NULL)
  {
  ...
  /* Copy Till The '.' */
  strncpy ((char *) pBkpFile, pFileName, uiLength - 3);

  /* Copy bkp as extention */
  strcpy ((char *) (pBkpFile + uiLength - 3), "bkp");

  CM_LDSV_DEBUG_PRINT ("pFileName= %s",pFileName);
  iRet = cmldsv_copy_file ((unsigned char *) pFileName, pBkpFile, pDirPath);
  of_free(pBkpFile);
  return iRet;
}
                          ...
                          char * pDirPath)
{
  ...
  while ((iBytes = cm_file_read (iSrcFsId, iSrcFd, (unsigned char *) pBuffer, LDSV_COPY_FILE_BUFFER_SIZE)) > 0)
  {
    if (cm_file_write (iDestFsId, iDestFd, (unsigned char *) pBuffer, iBytes) != iBytes)
    {
...
int32_t cm_ldsv_validate_file_name (unsigned char * pFileName)
{
  ...
  uiLength = strlen ((char *) pFileName);
  ...
  if (strcmp ((char *) pFileName + uiLength - 3, "xml"))
  {
    CM_LDSV_DEBUG_PRINT ("Invalid file name ");
    return OF_FAILURE;
  }
...

{
  ...
  iDestFd = cm_file_open (iDestFd, (unsigned char *) pFullPath, UCMFILE_MODE_WRITE);
  if (iDestFd <0)   //CID 163
  {
    //File not found, creating  new file
    if (errno == ENOENT)
    {
      iDestFd = cm_file_create (iDestFd, (unsigned char *) pFileName);
      if (iDestFd == -1)
      {
  ...
  iRet = cm_file_write (iDestFd, iDestFd, (unsigned char *) aBuffer, strlen (aBuffer));
...
int32_t cm_ldsv_close_open_xml_tags (char* pFileName)
{
  ...
  iDestFd = cm_file_open (iDestFd, (unsigned char *) pFileName, UCMFILE_MODE_WRITE_APPEND);
  if (iDestFd == -1)
  {
    CM_LDSV_DEBUG_PRINT (" Unable to open file ");
    return OF_FAILURE;
 ...
 int32_t cm_ldsv_write_version_number_to_file(char* pFullPath, uint64_t iVersionNo)
{
  ...
  {
    cm_ldsv_get_version_number_from_file(pFile, aPrevSavedVersionNo, &ullPrevSavedConfigVer);
    CM_LDSV_DEBUG_PRINT ("Failed to retrive previous saved config version number- returned: ");
    fclose(pFile);
  }
  ...
  strcat (aBuffer, aTmpBuffer);
  strcat (aBuffer, "\n\t\t</row>");*/
  sprintf(aBuffer,"%ld.%ld",(long int)ullTempEngVer,(long int)ullTempSaveVer);
...
int32_t cm_ldsv_get_version_number_from_file (FILE *pFile, char* pVersionNo, uint64_t *pullVersion)
{
  ...
  *pullVersion = 0;
  ...
  pResultArry = (unsigned char *) fgets (aReadLine, LDSV_XML_READ_LINE, pFile);
  if (pResultArry == NULL)
  {
    CM_LDSV_DEBUG_PRINT ("Nothing to load, may be empty data file");
    return OF_FAILURE;
  ...
  strcpy(aTempVersion,(char *) pResultArry);
 
  strcpy(pVersionNo, cm_ldsv_trim_string(aTempVersion));
  if((sscanf((char *) pResultArry, "%lu.%lu",(long unsigned int *) &uiSavedEngConfigVer, (long unsigned int *) &uiSavedConfigVer))< 2)
  {
    return OF_FAILURE;
  }
                          ...
                          struct cm_je_config_session *pConfigSess)
{
  ...
  if (pTablInTabHashTable == NULL)
  {
    ...
    return OF_FAILURE;
          ...
          CM_LDSV_DEBUG_PRINT ("aDmPath=%s,pInstPathFromFile=%s:",aDmPath,pInstPathFromFile);
