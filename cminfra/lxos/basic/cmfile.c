...
{
  return open ((char *) file_name_p, lMode, S_IRWXU);
}
...
{
  return creat ((char *) file_name_p, S_IRWXU);
}
...
{
  return remove ((char *) file_name_p);
}
...
{
  if (rename ((char *) pOldFileName, (char *) pNewFileName) == 0)
  {
    return OF_SUCCESS;
  }
  else
