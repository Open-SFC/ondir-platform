                           ...
                           uint32_t * remote_ip_p, uint16_t * remote_port_ui_p)
{
  ...
  if (bStoreAddress)
  {
    return_value = recvfrom (sock_fd_i, buffer_p, buf_len_i, flags_i,
                        (struct sockaddr *) &RemoteAddress, (socklen_t *) &iAddrLength);
  }
  else
                               ...
                               uint32_t * remote_ip, uint16_t * remote_port_ui)
{
  ...
  if ((iNewFd =
       accept (sock_fd_i, (struct sockaddr *) &rmtAddr, (socklen_t *) &rmtAddrLen)) < 0)
  {
    return OF_FAILURE;
  }
  else
                              ...
                              char ** remote_host_c, char ** remote_ip_a)
{
  ...
  *remote_ip_a = NULL;
  ...
  /* Get the peer's info */
  if ((getpeername (stream_sock_i, &addr, (socklen_t *) &len)) < 0)
  {
    return OF_FAILURE;
  }
...
{     
         ...
         return(getsockopt(iSockid,level_i,opt_name_i,(void*)opt_value_p,(socklen_t *) &iOptlen));
...
int32_t cm_socket_get_sock_name (cm_sock_handle fd_i, uint32_t * ip_p, uint16_t * port_p)
{
  ...
  if (getsockname (fd_i, (struct sockaddr *) &MyAddr, (socklen_t *) &MyAddrLen) < 0)
  {
    return OF_FAILURE;
  }
...
int32_t cm_socket_get_peer_name (cm_sock_handle fd_i, uint32_t * ip_p, uint16_t * port_p)
{
  ...
  if (getpeername (fd_i, (struct sockaddr *) &MyAddr, (socklen_t *) &MyAddrLen) < 0)
  {
    return OF_FAILURE;
  }
