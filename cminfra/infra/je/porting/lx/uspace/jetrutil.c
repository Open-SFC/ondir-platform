                             ...
                             struct cm_msg_generic_header * gen_hdr_p)
{
  ...
  {
    of_strcpy ((char *) gen_hdr_p->admin_name, role_info_p->admin_name);
    CM_JETRANS_DEBUG_PRINT ("AdminName=%s",gen_hdr_p->admin_name);
  }
  /* Admin Role  */
  if (role_info_p->admin_role[0] != '\0')
  {
    of_strcpy ((char *) gen_hdr_p->admin_role, role_info_p->admin_role);
...
//  uiLen += uiInt32Size_g;
  ...
  if (dm_path_p)
  {
    uiDMPathLen = of_strlen ((char *) dm_path_p);
    uiLen += uiDMPathLen;
  }
                                     ...
                                     unsigned char * pSendMsg)
{
  ...
  {
    uiDMPathLen = of_strlen ((char *) pReqMsg->dm_path_p);
                                    ...
                                    struct cm_dm_node_info ** node_info_pp)
{
  ...
  uint32_t uicFileLen = 0,uiCmdLen = 0,jj = 0,uiIpLen = 0;
