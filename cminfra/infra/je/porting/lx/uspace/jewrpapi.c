      ...
      struct cm_result ** presult_p)
{
   ...
   of_strncpy (role_info.admin_role, (char *) pJEConfigSession->admin_role,
         of_strlen ((char *) pJEConfigSession->admin_role));
   of_strncpy (role_info.admin_name, (char *) pJEConfigSession->admin_name,
         of_strlen ((char *) pJEConfigSession->admin_name));
   ...
   cm_fill_request_message (pUCMRequestMsg, command_p->command_id,
         (unsigned char *) command_p->dm_path_p, 0, &(command_p->nv_pair_array), NULL,
         &uiMsgLen);
   ...
   if ((cm_tnsprt_send_message (pJEConfigSession->tnsprt_channel_p,
               &UCMGenericHeader, (char *) pSocketBuf,
               uiMsgLen)) != OF_SUCCESS)
   ...
   if ((UCMExtractSessionResponseFromBuff
            (CM_COMMAND_CONFIG_SESSION_UPDATE, (char *) pSocketBuf, &uiMsgLen,
      ...
      struct cm_result ** presult_p)
{
   ...
   /*Fill Request Message Structure */
   cm_fill_request_message (pUCMRequestMsg, command_p->command_id,
         (unsigned char *) command_p->dm_path_p, 0, &(command_p->nv_pair_array),
         NULL, &uiMsgLen);
   ...
   /*Fill Response Msg from Recieve Buffer */
   if ((UCMExtractSessionResponseFromBuff
            (CM_COMMAND_CONFIG_SESSION_GET, (char *) pSocketBuf, &uiMsgLen,
      ...
      struct cm_result ** presult_p)
{
   ...
   if ((cm_tnsprt_send_message
            (pJEConfigSession->tnsprt_channel_p, &UCMGenericHeader, (char *) pSocketBuf,
             uiMsgLen)) != OF_SUCCESS)
   {
      CM_JETRANS_DEBUG_PRINT ("Sending through transport channel failed");
      ...
      void * tnsprt_channel_p)
{
   ...
   pNvPairAry->nv_pairs[0].value_length = of_strlen((char *) user_name_p);
   if (pNvPairAry->nv_pairs[0].value_length > 0)
   {
      ...
      of_strncpy (pNvPairAry->nv_pairs[0].value_p, (char *) user_name_p, pNvPairAry->nv_pairs[0].value_length);
   }
   ...
   pNvPairAry->nv_pairs[1].value_length = of_strlen((char *) password_p);
   if (pNvPairAry->nv_pairs[1].value_length > 0)
   {
      ...
      of_strncpy (pNvPairAry->nv_pairs[1].value_p, (char *) password_p, pNvPairAry->nv_pairs[1].value_length);
   }
      ...
      uint64_t end_version, uint32_t *ver_cnt_ui_p, struct cm_version **version_list_pp)
{
   ...
   pTmp = pSocketBuf;
   pTmp = (unsigned char *) of_mbuf_put_64 ((char *) pTmp + uiReqMsgLen, from_version_ui);

   /* Append from version  SocketBuffer */
   pTmp = (unsigned char *) of_mbuf_put_64 ((char *) pTmp, end_version);
   ...
   /*Fill Response Msg from Recieve Buffer */
   if ((UCMExtractSessionResponseFromBuff
            (CM_COMMAND_VERSION_HISTORY, (char *) pSocketBuf, &uiMsgLen,
      ...
      uint32_t notification_ui, void *notify_data_p)
{
   ...
   /* Append from version  SocketBuffer */
   pTmp = pSocketBuf;
   pTmp = (unsigned char *) of_mbuf_put_32 ((char *) pTmp + uiReqMsgLen, notification_ui);

   /* Append from version  SocketBuffer */
   pTmp = pSocketBuf;

   pTmp = (unsigned char *) of_mbuf_put_64 ((char *) pTmp + uiReqMsgLen+uiOSIntSize_g,version);
   ...
   /*Fill Response Msg from Recieve Buffer */
   if ((UCMExtractSessionResponseFromBuff
            (CM_COMMAND_NOTIFICATION, (char *) pSocketBuf, &uiMsgLen,
