      ...
      struct cm_je_config_session * pUCMCfgSession)
{
   ...
   of_strncpy ((char *) pUCMCfgSession->admin_role, (char *) pJECfgSession->admin_role,
         of_strlen((char *)pJECfgSession->admin_role));

   of_strncpy ((char *)pUCMCfgSession->admin_name, (char *) pJECfgSession->admin_name,
         of_strlen((char *) pJECfgSession->admin_name));
      ...
      struct cm_iv_pair * pOutIvPair)
{
   ...
   /* Input Name Value Pair Structure */
   if ((pInIvPair == NULL) || (pInIvPair->value_p == NULL) || (strlen(pInIvPair->value_p) ==0) )
   {
      ...
      uint32_t * puiCfgSessionLen)
{
   ...
   pUCMRespBuf = (char *)of_mbuf_put_32(pUCMRespBuf,of_strlen((char *) pUCMCfgSession->admin_role));

   of_memcpy (pUCMRespBuf, pUCMCfgSession->admin_role,of_strlen((char *) pUCMCfgSession->admin_role));
   pUCMRespBuf += of_strlen((char *) pUCMCfgSession->admin_role);

   pUCMRespBuf = (char *)of_mbuf_put_32(pUCMRespBuf,of_strlen((char *) pUCMCfgSession->admin_name));
   of_memcpy (pUCMRespBuf, pUCMCfgSession->admin_name,of_strlen((char *) pUCMCfgSession->admin_name));
   pUCMRespBuf += of_strlen((char *) pUCMCfgSession->admin_name);

   pUCMCfgSession->tnsprt_channel_p = NULL;
   *puiCfgSessionLen += (5 * CM_UINT_OS_SIZE) + of_strlen((char *) pUCMCfgSession->admin_role) + of_strlen((char *) pUCMCfgSession->admin_name);
   return OF_SUCCESS;
}
